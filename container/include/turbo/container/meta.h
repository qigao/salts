...
#define TURBO_META_HASH_MAP_METHODS(M,C) \
    ...
    M(FROM_ENTRIES, from, from, CONTAINER_INVALID_ARGUMENT, C) \
    M(DESTROY, destroy, destroy, _, C) \
    M(CLEAR, clear, clear, _, C) \
    M(PUT, put, put, _, C) \
    M(GET, get, get, _, C) \
...
#define TURBO_META_MULTIMAP_METHODS(M,C) \
    ...
    M(RESERVE, reserve, reserve, _, C) \
    M(PUT, put, put, _, C) \
    M(RAW_CONST_PTR_KEY, values_const, get_values_const, turbo_vec_t, C) \
    M(RAW_PTR_KEY, values, get_values, turbo_vec_t, C) \
...
#define TURBO_META_BPLUS_TREE_METHODS(M,C) \
...
#define TURBO_META_BTREE_METHODS(M,C) \
    M(INIT_KV_COMPARE, init, init, _, C) \
    M(FROM_ENTRIES, from, from, CONTAINER_INVALID_ARGUMENT, C) \
    M(INIT_WITH_ORDER_COMPARE, init_with_order, init_with_order, _, C) \
    M(DESTROY, destroy, destroy, _, C) \
