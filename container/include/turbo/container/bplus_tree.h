...
static inline int turbo_bplus_tree_align_up(size_t value, size_t alignment, size_t *out) {
  size_t remainder;
  if (out == NULL || alignment == 0U) return CONTAINER_INVALID_ARGUMENT;
  remainder = value % alignment;
  if (remainder == 0U) {
    *out = value;
    return CONTAINER_OK;
  }
  if (value > SIZE_MAX - (alignment - remainder)) return CONTAINER_OUT_OF_MEMORY;
...
static inline int turbo_bplus_tree_reserve(turbo_bplus_tree_t *tree, size_t min_capacity) {
  (void)min_capacity;
  if (!turbo_bplus_tree_is_valid(tree)) return CONTAINER_INVALID_ARGUMENT;
  return CONTAINER_OK;
}
...
static inline int turbo_bplus_tree_parent_insert(turbo_bplus_tree_t *tree, turbo_bplus_tree_node_t *parent,
  ...
  if (tree == NULL || parent == NULL || sep_key == NULL || right == NULL) return CONTAINER_INVALID_ARGUMENT;
...
static inline int turbo_bplus_tree_split_child(turbo_bplus_tree_t *tree, turbo_bplus_tree_node_t *parent,
  ...
  if (tree == NULL || parent == NULL || !turbo_bplus_tree_is_valid(tree)) return CONTAINER_INVALID_ARGUMENT;
  if (index > parent->num_keys) return CONTAINER_INVALID_ARGUMENT;

  left = parent->children[index];
  if (left == NULL || left->num_keys != tree->max_keys || parent->num_keys >= tree->max_keys) return CONTAINER_INVALID_ARGUMENT;
  ...
  if (!left->is_leaf) {
    ...
    right->parent = parent;
    if (turbo_bplus_tree_parent_insert(tree, parent, index, promoted, right) != CONTAINER_OK) {
    ...
    if (turbo_bplus_tree_parent_insert(
            tree, parent, index, turbo_bplus_tree_node_key(tree, right, 0U), right) != CONTAINER_OK) {
      turbo_bplus_tree_node_destroy(right);
      return CONTAINER_INVALID_ARGUMENT;
    }
  }
...
static inline int turbo_bplus_tree_put(turbo_bplus_tree_t *tree, const void *key, const void *value) {
  ...
  if (!turbo_bplus_tree_is_valid(tree) || key == NULL || value == NULL) return CONTAINER_INVALID_ARGUMENT;
  if (tree->root == NULL) {
  ...
  if (tree->root->num_keys == tree->max_keys) {
    ...
    if (new_root == NULL) return CONTAINER_OUT_OF_MEMORY;
    new_root->children[0U] = tree->root;
    tree->root->parent = new_root;
    if (turbo_bplus_tree_split_child(tree, new_root, 0U) != CONTAINER_OK) {
  ...
  while (!node->is_leaf) {
    ...
    if (child->num_keys == tree->max_keys) {
      if (turbo_bplus_tree_split_child(tree, node, index) != CONTAINER_OK) return CONTAINER_OUT_OF_MEMORY;
  ...
  if (node->num_keys == tree->max_keys) return CONTAINER_OUT_OF_MEMORY;
...
static inline int turbo_bplus_tree_remove_rebuild(turbo_bplus_tree_t *tree, const void *key, void *out_value) {
  ...
  if (!turbo_bplus_tree_is_valid(tree) || key == NULL) return CONTAINER_INVALID_ARGUMENT;
  if (tree->root == NULL) return CONTAINER_NOT_FOUND;
  if (turbo_bplus_tree_init_with_order(&replacement, tree->key_size, tree->value_size, tree->compare,
                                       tree->compare_ctx, tree->min_degree) != CONTAINER_OK) {
    return CONTAINER_OUT_OF_MEMORY;
  }
  node = turbo_bplus_tree_leftmost_leaf(tree);
  while (node != NULL) {
...
#define TURBO_BPLUS_TREE_DEFINE(name, key_type, value_type, compare_fn) \
  CMETA_CONTAINER2_DEFINE(name, key_type, value_type, turbo_bplus_tree_t, turbo_bplus_tree, CONTAINER_OK, compare_fn, TURBO_META_BPLUS_TREE_METHODS) \
  CMETA_CONTAINER2_RANGES_DEFINE(name, key_type, value_type, turbo_bplus_tree, key_at_const, value_at_const, \
