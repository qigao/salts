...
static inline int turbo_hash_set_init(turbo_hash_set_t *set, size_t key_size,
                                      turbo_hash_fn hash, turbo_hash_equal_fn equal, void *ctx) {
  return turbo_set_init(set, key_size, hash, equal, ctx);
}
static inline int turbo_hash_set_from_array(turbo_hash_set_t *set, const void *keys,
                                            size_t count, size_t key_size,
                                            turbo_hash_fn hash, turbo_hash_equal_fn equal, void *ctx) {
  return turbo_set_from_array(set, keys, count, key_size, hash, equal, ctx);
}
...
static inline int turbo_hash_set_reserve(turbo_hash_set_t *set, size_t capacity) { return turbo_set_reserve(set, capacity); }
static inline int turbo_hash_set_add(turbo_hash_set_t *set, const void *key) { return turbo_set_add(set, key); }
static inline bool turbo_hash_set_contains(const turbo_hash_set_t *set, const void *key) { return turbo_set_contains(set, key); }
static inline int turbo_hash_set_remove(turbo_hash_set_t *set, const void *key) { return turbo_set_remove(set, key); }
...
#define TURBO_HASH_SET_DEFINE(name, key_type) \
  CMETA_CONTAINER1_DEFINE(name, key_type, turbo_hash_set_t, turbo_hash_set, TURBO_OK, _, TURBO_META_HASH_SET_METHODS) \
