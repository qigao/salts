...
static inline int turbo_queue_init(turbo_queue_t *queue, size_t elem_size) {
  return queue == NULL ? TURBO_EINVAL : turbo_deque_init(&queue->raw, elem_size);
}
static inline int turbo_queue_from_array(turbo_queue_t *queue, const void *elements,
                                         size_t count, size_t elem_size) {
  return queue == NULL ? TURBO_EINVAL : turbo_deque_from_array(&queue->raw, elements, count, elem_size);
}
...
static inline int turbo_queue_reserve(turbo_queue_t *queue, size_t capacity) {
  return queue == NULL ? TURBO_EINVAL : turbo_deque_reserve(&queue->raw, capacity);
}
static inline int turbo_queue_push(turbo_queue_t *queue, const void *elem) {
  return queue == NULL ? TURBO_EINVAL : turbo_deque_push_back(&queue->raw, elem);
}
static inline int turbo_queue_pop(turbo_queue_t *queue, void *out_elem) {
  return queue == NULL ? TURBO_EINVAL : turbo_deque_pop_front(&queue->raw, out_elem);
}
...
#define TURBO_QUEUE_DEFINE(name, type) \
  CMETA_CONTAINER1_DEFINE(name, type, turbo_queue_t, turbo_queue, TURBO_OK, _, TURBO_META_QUEUE_METHODS) \
