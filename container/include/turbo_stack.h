...
static inline int turbo_stack_init(turbo_stack_t *stack, size_t elem_size) {
  return stack == NULL ? TURBO_EINVAL : turbo_vec_init(&stack->raw, elem_size);
}
static inline int turbo_stack_from_array(turbo_stack_t *stack, const void *elements,
                                         size_t count, size_t elem_size) {
  return stack == NULL ? TURBO_EINVAL : turbo_vec_from_array(&stack->raw, elements, count, elem_size);
}
...
static inline int turbo_stack_reserve(turbo_stack_t *stack, size_t capacity) {
  return stack == NULL ? TURBO_EINVAL : turbo_vec_reserve(&stack->raw, capacity);
}
static inline int turbo_stack_push(turbo_stack_t *stack, const void *elem) {
  return stack == NULL ? TURBO_EINVAL : turbo_vec_push(&stack->raw, elem);
}
static inline int turbo_stack_pop(turbo_stack_t *stack, void *out_elem) {
  return stack == NULL ? TURBO_EINVAL : turbo_vec_pop(&stack->raw, out_elem);
}
...
#define TURBO_STACK_DEFINE(name, type) \
  CMETA_CONTAINER1_DEFINE(name, type, turbo_stack_t, turbo_stack, TURBO_OK, _, TURBO_META_STACK_METHODS) \
