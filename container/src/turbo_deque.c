...
static int turbo_deque_grow_to(turbo_deque_t *deque, size_t min_capacity) {
  ...
  if (min_capacity <= deque->capacity) return CONTAINER_OK;
  if (min_capacity > SIZE_MAX / deque->elem_size) return CONTAINER_OUT_OF_MEMORY;
  ...
  if (new_capacity > SIZE_MAX / deque->elem_size) return CONTAINER_OUT_OF_MEMORY;
...
int turbo_deque_init(turbo_deque_t *deque, size_t elem_size) {
  if (!deque || elem_size == 0) return CONTAINER_INVALID_ARGUMENT;
...
int turbo_deque_from_array(turbo_deque_t *deque, const void *elements, size_t count,
  ...
  if (!deque || elem_size == 0 || (count > 0 && !elements)) return CONTAINER_INVALID_ARGUMENT;
...
int turbo_deque_push_back(turbo_deque_t *deque, const void *elem) {
  ...
  if (deque->size == SIZE_MAX) return CONTAINER_OUT_OF_MEMORY;
...
int turbo_deque_pop_back(turbo_deque_t *deque, void *out_elem) {
  ...
  if (!turbo_deque_valid(deque)) return CONTAINER_INVALID_ARGUMENT;
  if (deque->size == 0) return CONTAINER_EMPTY;
