...
static int turbo_heap_grow_to(turbo_heap_t *heap, size_t min_capacity) {
  ...
  if (min_capacity <= heap->capacity) return CONTAINER_OK;
  if (min_capacity > SIZE_MAX / heap->elem_size) return CONTAINER_OUT_OF_MEMORY;
  ...
  if (new_capacity > SIZE_MAX / heap->elem_size) return CONTAINER_OUT_OF_MEMORY;
...
int turbo_heap_init(turbo_heap_t *heap, size_t elem_size, turbo_heap_compare_fn compare,
                    void *compare_ctx) {
  if (!heap || elem_size == 0 || !compare) return CONTAINER_INVALID_ARGUMENT;
...
int turbo_heap_from_array(turbo_heap_t *heap, const void *elements, size_t count,
                          size_t elem_size, turbo_heap_compare_fn compare,
                          void *compare_ctx) {
  size_t parent;
  int rc;

  if (!heap || elem_size == 0 || !compare || (count > 0 && !elements)) return CONTAINER_INVALID_ARGUMENT;
  rc = turbo_heap_init(heap, elem_size, compare, compare_ctx);
  if (rc != CONTAINER_OK) return rc;
  rc = turbo_heap_reserve(heap, count);
  if (rc != CONTAINER_OK) {
  ...
  for (parent = count / 2U; parent > 0; --parent) {
    rc = turbo_heap_sift_down(heap, parent - 1U);
    if (rc != CONTAINER_OK) {
      turbo_heap_destroy(heap);
      return rc;
    }
...
int turbo_heap_push(turbo_heap_t *heap, const void *elem) {
  ...
  if (heap->size == SIZE_MAX) return CONTAINER_OUT_OF_MEMORY;
...
int turbo_heap_pop(turbo_heap_t *heap, void *out_elem) {
  if (!turbo_heap_valid(heap)) return CONTAINER_INVALID_ARGUMENT;
  if (heap->size == 0) return CONTAINER_EMPTY;
