...
static int rebuild_without(turbo_btree_t *tree, const void *remove_key, void *out_value) {
  ...
  for (i = 0U; i < tree->size; ++i) {
    ...
    if (rc != TURBO_OK) { turbo_btree_destroy(&next); return rc; }
  }
  if (!removed) { turbo_btree_destroy(&next); return TURBO_ENOENT; }
  node_destroy(tree->root);
  tree->root = next.root;
  tree->size = next.size;
  next.root = NULL;
...
int turbo_btree_remove(turbo_btree_t *tree, const void *key, void *out_value) {
  ...
  if (!turbo_btree_contains(tree, key)) return TURBO_ENOENT;
...
int turbo_btree_from_arrays(turbo_btree_t *tree, const void *keys, const void *values,
  ...
  for (i = 0U; i < count; ++i) {
    ...
    if (rc != TURBO_OK) { turbo_btree_destroy(tree); return rc; }
  }
  return TURBO_OK;
}
