...
container_status turbo_hash_set_init(turbo_hash_set_t *set,
...
container_status turbo_hash_set_init_bytes(
...
static container_status turbo_hash_set_from_common(
...
container_status turbo_hash_set_from_array(
    turbo_hash_set_t *set, const void *keys, size_t count,
...
container_status turbo_hash_set_from_array_bytes(
...
container_status turbo_hash_set_reserve(turbo_hash_set_t *set,
                                        size_t min_entries) {
  return set == NULL ? CONTAINER_INVALID_ARGUMENT
                     : turbo_hash_map_reserve(&set->table, min_entries);
}

container_status turbo_hash_set_add(turbo_hash_set_t *set,
                                    const void *key) {
  uint8_t present = 1u;
  if (set == NULL) return CONTAINER_INVALID_ARGUMENT;
  if (turbo_hash_map_contains(&set->table, key)) return CONTAINER_OK;
...
container_status turbo_hash_set_remove(turbo_hash_set_t *set,
                                       const void *key) {
  return set == NULL ? CONTAINER_INVALID_ARGUMENT
