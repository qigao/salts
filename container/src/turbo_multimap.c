...
static container_status turbo_multimap_initialize(
...
container_status turbo_multimap_init(
    turbo_multimap_t *map, const cmeta_type_desc *key_type,
...
container_status turbo_multimap_init_bytes(
...
static container_status turbo_multimap_from_common(
...
container_status turbo_multimap_from_arrays(
    turbo_multimap_t *map, const void *keys, const void *values, size_t count,
    const cmeta_type_desc *key_type, const cmeta_type_desc *value_type,
...
container_status turbo_multimap_from_arrays_bytes(
    turbo_multimap_t *map, const void *keys, const void *values, size_t count,
    size_t key_size, size_t key_align, size_t value_size, size_t value_align,
    size_t element_limit, turbo_multimap_compare_fn compare, void *context) {
  return turbo_multimap_from_common(
...
container_status turbo_multimap_put(turbo_multimap_t *map, const void *key,
                                    const void *value) {
  turbo_rbtree_t *tree = turbo_multimap_tree(map);
...
bool turbo_multimap_remove(turbo_multimap_t *map, const void *key,
  ...
  if (turbo_rbtree_remove_node(tree, node, out_value) != CONTAINER_OK)
    return false;
  ++map->generation;
  return true;
}
...
size_t turbo_multimap_erase(turbo_multimap_t *map, const void *key) {
  ...
  while (node != end) {
    ...
    if (turbo_rbtree_remove_node(tree, node, NULL) != CONTAINER_OK) break;
    ++removed;
    node = next;
  }
...
container_status turbo_multimap_iter_next(turbo_multimap_iter_t *iterator) {
  turbo_rbtree_node_t *node;
...
container_status turbo_multimap_iter_prev(turbo_multimap_iter_t *iterator) {
