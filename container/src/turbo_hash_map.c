...
static int turbo_hash_add_size(size_t left, size_t right, size_t *out) {
  if (right > SIZE_MAX - left) return CONTAINER_OUT_OF_MEMORY;
  *out = left + right;
  return CONTAINER_OK;
}
...
static int turbo_hash_align_size(size_t value, size_t alignment, size_t *out) {
  ...
  return remainder == 0 ? (*out = value, CONTAINER_OK)
...
static int turbo_hash_alloc_arrays(turbo_hash_map_t *map, size_t capacity) {
  ...
  if (capacity > SIZE_MAX / map->key_size) return CONTAINER_OUT_OF_MEMORY;
  if (capacity > SIZE_MAX / map->value_size) return CONTAINER_OUT_OF_MEMORY;
  if (capacity > SIZE_MAX / sizeof(size_t)) return CONTAINER_OUT_OF_MEMORY;

  total_size = capacity * sizeof(uint8_t);
  if (turbo_hash_align_size(total_size, alignment, &hashes_offset) != CONTAINER_OK)
    return CONTAINER_OUT_OF_MEMORY;
  region_size = capacity * sizeof(size_t);
  if (turbo_hash_add_size(hashes_offset, region_size, &total_size) != CONTAINER_OK)
    return CONTAINER_OUT_OF_MEMORY;
  if (turbo_hash_align_size(total_size, alignment, &keys_offset) != CONTAINER_OK)
    return CONTAINER_OUT_OF_MEMORY;
  region_size = capacity * map->key_size;
  if (turbo_hash_add_size(keys_offset, region_size, &total_size) != CONTAINER_OK)
    return CONTAINER_OUT_OF_MEMORY;
  if (turbo_hash_align_size(total_size, alignment, &values_offset) != CONTAINER_OK)
    return CONTAINER_OUT_OF_MEMORY;
  region_size = capacity * map->value_size;
  if (turbo_hash_add_size(values_offset, region_size, &total_size) != CONTAINER_OK)
    return CONTAINER_OUT_OF_MEMORY;

  storage = (unsigned char *)malloc(total_size);
  if (!storage) return CONTAINER_OUT_OF_MEMORY;
...
static int turbo_hash_map_ensure_load(turbo_hash_map_t *map, size_t extra) {
  ...
  if (extra > SIZE_MAX - map->size) return CONTAINER_OUT_OF_MEMORY;
  wanted = map->size + extra;
  if (map->size > SIZE_MAX - map->tombstones) return CONTAINER_OUT_OF_MEMORY;
  if (extra > SIZE_MAX - map->size - map->tombstones) return CONTAINER_OUT_OF_MEMORY;
  used = map->size + map->tombstones + extra;
  if (map->capacity != 0 && used <= map->capacity - (map->capacity / 10U) * 3U) {
    return CONTAINER_OK;
  }

  if (wanted > (SIZE_MAX - 1U) / 10U) return CONTAINER_OUT_OF_MEMORY;
  min_slots = (wanted * 10U) / 7U + 1U;
  capacity = turbo_hash_round_pow2(min_slots);
  if (capacity == 0) return CONTAINER_OUT_OF_MEMORY;
...
int turbo_hash_map_reserve(turbo_hash_map_t *map, size_t min_capacity) {
  ...
  if (min_capacity == 0) return CONTAINER_OK;
  if (min_capacity > (SIZE_MAX - 1U) / 10U) return CONTAINER_OUT_OF_MEMORY;
  min_slots = (min_capacity * 10U) / 7U + 1U;
  capacity = turbo_hash_round_pow2(min_slots);
  if (capacity == 0) return CONTAINER_OUT_OF_MEMORY;
  if (capacity <= map->capacity) return CONTAINER_OK;
  return turbo_hash_map_rehash(map, capacity);
}
...
int turbo_hash_map_put(turbo_hash_map_t *map, const void *key, const void *value) {
  ...
  if (!turbo_hash_map_valid(map) || !key || !value) return CONTAINER_INVALID_ARGUMENT;
  ...
  if (map->capacity > SIZE_MAX / 2U) return CONTAINER_OUT_OF_MEMORY;
...
int turbo_hash_map_remove(turbo_hash_map_t *map, const void *key, void *out_value) {
  ...
  if (!turbo_hash_map_valid(map) || !key) return CONTAINER_INVALID_ARGUMENT;
  hash = map->hash(key, map->key_size, map->ctx);
  if (hash == 0) hash = 1;
