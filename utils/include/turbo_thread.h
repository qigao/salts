 ...
 *   }
 */
TURBO_C_API void turbo_mutex_init(turbo_mutex_t *mutex);

/**
 * @brief Destroy a mutex
 * @param mutex Mutex to destroy
 */
TURBO_C_API void turbo_mutex_destroy(turbo_mutex_t *mutex);

/**
 * @brief Lock a mutex
 * @param mutex Mutex to lock
 */
TURBO_C_API void turbo_mutex_lock(turbo_mutex_t *mutex);

/**
 * @brief Unlock a mutex
 * @param mutex Mutex to unlock
 */
TURBO_C_API void turbo_mutex_unlock(turbo_mutex_t *mutex);
 ...
 */
TURBO_C_API int turbo_rwlock_init(turbo_rwlock_t *lock);

/**
 * @brief Destroy a read-write lock
 * @param lock Lock to destroy
 */
TURBO_C_API void turbo_rwlock_destroy(turbo_rwlock_t *lock);

/**
 * @brief Acquire the read lock (shared, multiple readers allowed)
 * @param lock Lock to acquire
 */
TURBO_C_API void turbo_rwlock_rdlock(turbo_rwlock_t *lock);

/**
 * @brief Release the read lock
 * @param lock Lock to release
 */
TURBO_C_API void turbo_rwlock_rdunlock(turbo_rwlock_t *lock);

/**
 * @brief Acquire the write lock (exclusive, blocks all readers and writers)
 * @param lock Lock to acquire
 */
TURBO_C_API void turbo_rwlock_wrlock(turbo_rwlock_t *lock);

/**
 * @brief Release the write lock
 * @param lock Lock to release
 */
TURBO_C_API void turbo_rwlock_wrunlock(turbo_rwlock_t *lock);
 ...
 */
TURBO_C_API void turbo_cond_wait(turbo_cond_t *cond, turbo_mutex_t *mutex);
 ...
 */
TURBO_C_API int turbo_cond_timedwait(turbo_cond_t *cond, turbo_mutex_t *mutex, uint64_t timeout_ns);
 ...
 *   }
 */
TURBO_C_API int turbo_thread_create(turbo_thread_t *thread, turbo_thread_cb entry, void *arg);
 ...
 * @return 0 on success, < 0 on failure
 */
TURBO_C_API int turbo_thread_join(turbo_thread_t *thread);

/**
 * @brief Destroy a thread handle (detach if running)
 * @param thread Thread handle
 */
TURBO_C_API void turbo_thread_destroy(turbo_thread_t *thread);
 ...
 */
TURBO_C_API int turbo_threadpool_submit(turbo_threadpool_t *pool, turbo_task_fn task, void *arg);
 ...
 */
TURBO_C_API int turbo_threadpool_try_submit(turbo_threadpool_t *pool, turbo_task_fn task,
                                          void *arg);
