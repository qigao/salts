 ...
 */
TURBO_C_API int mem_init(mem_pool_t *pool, size_t initial_size);

/**
 * @brief Get global shared slab pool (lazy init)
 * @return Pointer to global pool
 */
TURBO_C_API mem_pool_t *mem_global(void);

/**
 * @brief Destroy pool and free all memory
 * @param pool Pool structure
 */
TURBO_C_API void mem_destroy(mem_pool_t *pool);
 ...
 */
TURBO_C_API void mem_free(mem_pool_t* pool, void* ptr);
/**
 ...
 */
TURBO_C_API void mem_reset(mem_pool_t *pool);

/**
 * @brief Trim empty slabs
 * @param pool Pool structure
 */
TURBO_C_API void mem_trim(mem_pool_t *pool);
 ...
 */
TURBO_C_API void *mem_alloc(mem_pool_t *pool, size_t size);
 ...
 */
TURBO_C_API void *mem_alloc_array(mem_pool_t *pool, size_t element_size, size_t count);
 ...
 */
TURBO_C_API char *mem_strdup(mem_pool_t *pool, const char *str);
 ...
 */
TURBO_C_API char *mem_sprintf(mem_pool_t *pool, const char *fmt, ...);
 ...
 */
TURBO_C_API mem_buffer_t *mem_get_buffer(mem_pool_t *pool, size_t min_size);

/**
 * @brief Release one shared buffer reference
 * @param buffer Buffer instance
 */
TURBO_C_API void mem_release(mem_buffer_t *buffer);

/**
 * @brief Increment buffer reference count
 * @param buffer Buffer instance
 */
TURBO_C_API void mem_ref(mem_buffer_t *buffer);

/**
 * @brief Decrement buffer reference count (recycle when reaches 0)
 * @param buffer Buffer instance
 */
TURBO_C_API void mem_unref(mem_buffer_t *buffer);
 ...
 */
TURBO_C_API mem_buffer_t *mem_buffer_retain(mem_buffer_t *buffer);

/**
 * @brief Release one shared buffer reference
 * @param buffer Buffer instance
 */
TURBO_C_API void mem_buffer_release(mem_buffer_t *buffer);
 ...
 */
TURBO_C_API mem_buffer_t *mem_wrap_external(void *data, size_t size,
 ...
 */
TURBO_C_API int mem_is_external(const mem_buffer_t *buffer);

TURBO_C_API size_t mem_pool_total_allocated(const mem_pool_t *pool);
TURBO_C_API size_t mem_pool_total_used(const mem_pool_t *pool);
/** @brief Return a concurrent snapshot of cached internal buffer shells. */
TURBO_C_API size_t mem_pool_recycle_count(const mem_pool_t *pool);
TURBO_C_API char *mem_buffer_data(mem_buffer_t *buffer);
TURBO_C_API const char *mem_buffer_const_data(const mem_buffer_t *buffer);
TURBO_C_API size_t mem_buffer_capacity(const mem_buffer_t *buffer);
TURBO_C_API size_t mem_buffer_used(const mem_buffer_t *buffer);
TURBO_C_API uint32_t mem_buffer_ref_count(const mem_buffer_t *buffer);
TURBO_C_API const mem_pool_t *mem_buffer_pool(const mem_buffer_t *buffer);
 ...
 */
TURBO_C_API mem_slice_t mem_slice(mem_buffer_t *buffer, size_t offset, size_t length);

/**
 * @brief Release slice (decrements buffer refcount)
 * @param slice Slice instance
 */
TURBO_C_API void mem_slice_release(mem_slice_t *slice);
