 ...
 * @return TURBO_OK on success, otherwise a TURBO_* error code.
 *
 ...
 * turbo_zstd_min_level() and turbo_zstd_max_level(), inclusive. A NULL source
 ...
 * @return TURBO_OK on success, TURBO_ENOSPC if the destination is too small,
 *         or another TURBO_* error code.
 *
 ...
 * @return TURBO_OK on success, TURBO_ENOSPC if the destination is too small,
 *         TURBO_EPROTO for malformed or unsupported frames, or another
 *         TURBO_* error code.
 *
 * @note The function is thread-safe. On failure, @p out_size is unchanged.
 */
 ...
 * @return TURBO_OK on success, TURBO_ENOTSUP when the frame does not declare a
 *         content size, TURBO_ERANGE when it cannot fit in size_t, or another
 *         TURBO_* error code.
 *
