 ...
 */
SALTS_C_API int mdns_publish(mdns_ctx_t *ctx, const mdns_service_t *service);
/**
 ...
 * @return 0 on success, or a non-zero error code on failure.
 */
SALTS_C_API int mdns_unpublish(mdns_ctx_t *ctx, const char *instance, const char *service_type);
 ...
 */
SALTS_C_API int mdns_discover(mdns_ctx_t *ctx, const char *service_type, mdns_discover_cb callback,
 ...
 */
SALTS_C_API const char *mdns_get_local_hostname(void);
/**
 * @brief Retrieves the local IP address.
 *
 * @return A string containing the local IP address.
 */
SALTS_C_API const char *mdns_get_local_ip(void);
