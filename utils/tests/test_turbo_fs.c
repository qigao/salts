...
spec("Turbo FS Tests") {
  ...
  describe("Bulk read/write") {
    ...
    it("write_file with NULL buf returns -EINVAL") {
      check_equal(turbo_fs_write_file(g_file, NULL), -EINVAL);
    }
  }
  ...
  describe("stat") {
    it("stat on written file reports correct size and is_file") {
      ...
      check_equal(turbo_fs_stat(g_file, &st), 0);
    ...
    it("stat on missing path returns negative errno") {
      turbo_fs_stat_t st = {0};
      check_less(turbo_fs_stat("__ghost__", &st), 0);
    }
    ...
    it("chmod and access expose basic permission controls") {
      ...
      check_equal(turbo_fs_access(g_file, TURBO_FS_ACCESS_EXISTS), 0);
      check_equal(turbo_fs_access(g_file, TURBO_FS_ACCESS_READ), 0);
      check_equal(turbo_fs_chmod(g_file, 0444), 0);
      check_equal(turbo_fs_access(g_file, TURBO_FS_ACCESS_READ), 0);
      check_equal(turbo_fs_chmod(g_file, 0644), 0);
    }
  }
  ...
  describe("symlink") {
    it("creates and inspects symlinks when the platform permits it") {
      ...
      if (rc == 0) {
        turbo_fs_stat_t st = {0};
        check_equal(turbo_fs_lstat(g_link, &st), 0);
        ...
        check_equal(turbo_fs_unlink(g_link), 0);
      } else {
        check_less(rc, 0);
      }
    }
  ...
  describe("mkdir / rmdir / unlink") {
    it("mkdir creates a directory, stat sees it as directory") {
      check_equal(turbo_fs_mkdir(g_dir, 0755), 0);

      turbo_fs_stat_t st = {0};
      check_equal(turbo_fs_stat(g_dir, &st), 0);
      check_equal((int)st.is_directory, 1);
    }

    it("rmdir on empty dir succeeds") {
      turbo_fs_mkdir(g_dir, 0755);
      check_equal(turbo_fs_rmdir(g_dir), 0);
    }

    it("unlink removes a file") {
      turbo_fs_buf_t wb = turbo_fs_buf_init((char *)"x", 1);
      check_equal(turbo_fs_write_file(g_file, &wb), 0);
      check_equal(turbo_fs_unlink(g_file), 0);

      turbo_fs_stat_t st = {0};
      check_less(turbo_fs_stat(g_file, &st), 0);
    }

    it("unlink rejects directories and leaves them intact") {
      turbo_fs_rmdir(g_dir);
      check_equal(turbo_fs_mkdir(g_dir, 0755), 0);
      check_less(turbo_fs_unlink(g_dir), 0);
  ...
  describe("directory enumeration") {
    it("enumerates child names and available types") {
      ...
      check_equal(turbo_fs_path_join(file_path, sizeof(file_path), g_dir, "child.txt"), 0);
      check_equal(turbo_fs_path_join(child_dir_path, sizeof(child_dir_path), g_dir, "nested"), 0);
      ...
      check_equal(turbo_fs_mkdir(g_dir, 0755), 0);
      turbo_fs_buf_t wb = turbo_fs_buf_init((char *)"x", 1);
      check_equal(turbo_fs_write_file(file_path, &wb), 0);
      check_equal(turbo_fs_mkdir(child_dir_path, 0755), 0);
      ...
      if (open_rc == 0) {
        ...
        check_equal(turbo_fs_closedir(dir), 0);
      }

      check_equal(turbo_fs_unlink(file_path), 0);
      check_equal(turbo_fs_rmdir(child_dir_path), 0);
      check_equal(turbo_fs_rmdir(g_dir), 0);
    }
    ...
    it("reports end immediately for an empty directory") {
      ...
      if (open_rc == 0) {
        turbo_fs_dirent_t entry;
        check_equal(turbo_fs_readdir(dir, &entry), 0);
        check_equal(turbo_fs_closedir(dir), 0);
      }
      check_equal(turbo_fs_rmdir(g_dir), 0);
    }
    ...
    it("rejects invalid arguments and missing paths") {
      turbo_fs_dir_t *dir = NULL;
      turbo_fs_dirent_t entry;
      check_equal(turbo_fs_opendir(NULL, &dir), -EINVAL);
      check_null(dir);
      check_equal(turbo_fs_opendir(g_dir, NULL), -EINVAL);
      check_equal(turbo_fs_readdir(NULL, &entry), -EINVAL);
      check_equal(turbo_fs_closedir(NULL), -EINVAL);
      check_equal(turbo_fs_opendir("__missing_directory__", &dir), -ENOENT);
      check_null(dir);
    }
  }
  ...
  describe("rename") {
    it("rename moves file to new path") {
      ...
      check_equal(turbo_fs_write_file(g_file, &wb), 0);
      check_equal(turbo_fs_rename(g_file, g_file2), 0);

      turbo_fs_stat_t st = {0};
      check_less(turbo_fs_stat(g_file, &st),  0); /* old gone */
      check_equal(turbo_fs_stat(g_file2, &st), 0); /* new exists */
    }
    ...
    it("rename replaces an existing destination file") {
      ...
      check_equal(turbo_fs_write_file(g_file, &source), 0);
      check_equal(turbo_fs_write_file(g_file2, &destination), 0);
      check_equal(turbo_fs_rename(g_file, g_file2), 0);
      check_less(turbo_fs_access(g_file, TURBO_FS_ACCESS_EXISTS), 0);
      check_equal(turbo_fs_read_file(g_file2, &actual), 0);
  ...
  describe("Streaming operations") {
    it("open/write/close then open/read/close round-trips") {
      ...
      check_not_equal(fd, TURBO_INVALID_FILE);
      check_equal(turbo_fs_write(fd, data, len), (int)len);
      check_equal(turbo_fs_close(fd), 0);

      fd = turbo_fs_open(g_file, TURBO_FS_O_RDONLY, 0);
      check_not_equal(fd, TURBO_INVALID_FILE);

      char buf[64] = {0};
      check_equal(turbo_fs_read(fd, buf, sizeof(buf)), (int)len);
    ...
    it("open on missing file without O_CREAT returns TURBO_INVALID_FILE") {
      ...
      check_equal(fd, TURBO_INVALID_FILE);
    }

    it("read/write on TURBO_INVALID_FILE return -EINVAL") {
      char buf[8] = {0};
      check_equal(turbo_fs_read(TURBO_INVALID_FILE, buf, 8),  -EINVAL);
      check_equal(turbo_fs_write(TURBO_INVALID_FILE, buf, 8), -EINVAL);
    }
  }
  ...
  describe("pread / pwrite") {
    it("pwrite at offset then pread retrieves correct slice") {
      ...
      check_equal(turbo_fs_pwrite(fd, "AAAAA", 5, 0), 5);
      check_equal(turbo_fs_pwrite(fd, "BBBBB", 5, 5), 5);

      char buf[6] = {0};
      check_equal(turbo_fs_pread(fd, buf, 5, 5), 5);
    ...
    it("pread and pwrite preserve the current file position") {
      turbo_file_t fd = turbo_fs_open(g_file,
          TURBO_FS_O_RDWR | TURBO_FS_O_CREAT | TURBO_FS_O_TRUNC, 0644);
      check_not_equal(fd, TURBO_INVALID_FILE);
      check_equal(turbo_fs_write(fd, "abcdef", 6), 6);
      check_equal((int)turbo_fs_seek(fd, 3, SEEK_SET), 3);

      char ch = 0;
      check_equal(turbo_fs_pread(fd, &ch, 1, 0), 1);
      check_equal(ch, 'a');
      check_equal((int)turbo_fs_tell(fd), 3);

      check_equal(turbo_fs_pwrite(fd, "Z", 1, 1), 1);
      check_equal((int)turbo_fs_tell(fd), 3);
      check_equal(turbo_fs_close(fd), 0);
  ...
  describe("streaming bounds") {
    it("rejects lengths that cannot be represented by the int return type") {
      ...
      check_equal(turbo_fs_write(fd, &byte, too_large), -EOVERFLOW);
      check_equal(turbo_fs_pwrite(fd, &byte, too_large, 0), -EOVERFLOW);
      check_equal(turbo_fs_read(fd, &byte, too_large), -EOVERFLOW);
      check_equal(turbo_fs_pread(fd, &byte, too_large, 0), -EOVERFLOW);
  ...
  describe("file locks") {
    it("locks and unlocks a byte range") {
      ...
      check_not_equal(fd, TURBO_INVALID_FILE);
      check_equal(turbo_fs_lock(fd, TURBO_FS_LOCK_EXCLUSIVE, 0, 0), 0);
      check_equal(turbo_fs_unlock(fd, 0, 0), 0);
      check_equal(turbo_fs_close(fd), 0);
    }
    ...
    it("rejects invalid lock arguments") {
      turbo_file_t fd = turbo_fs_open(g_file,
          TURBO_FS_O_RDWR | TURBO_FS_O_CREAT | TURBO_FS_O_TRUNC, 0644);
      check_not_equal(fd, TURBO_INVALID_FILE);
      check_equal(turbo_fs_lock(fd, TURBO_FS_LOCK_SHARED | TURBO_FS_LOCK_EXCLUSIVE, 0, 1),
                   -EINVAL);
      check_equal(turbo_fs_lock(fd, 0, 0, 1), -EINVAL);
      check_equal(turbo_fs_lock(fd, TURBO_FS_LOCK_SHARED, -1, 1), -EINVAL);
      check_equal(turbo_fs_close(fd), 0);
    }
  }
  ...
  describe("seek / tell") {
    it("seek to beginning, tell reports 0") {
      ...
      check_equal((int)turbo_fs_seek(fd, 5, SEEK_SET), 5);
      check_equal((int)turbo_fs_tell(fd), 5);

      check_equal((int)turbo_fs_seek(fd, 0, SEEK_SET), 0);
      check_equal((int)turbo_fs_tell(fd), 0);
  ...
  describe("ftruncate") {
    it("truncate shortens file to specified length") {
      ...
      turbo_file_t fd = turbo_fs_open(g_file, TURBO_FS_O_RDWR, 0);
      check_not_equal(fd, TURBO_INVALID_FILE);
      check_equal(turbo_fs_ftruncate(fd, 5), 0);
      turbo_fs_close(fd);
  ...
  describe("fsync") {
    it("fsync on valid fd returns 0") {
      ...
      check_equal(turbo_fs_fsync(fd), 0);
      turbo_fs_close(fd);
    }
  }
  ...
  describe("tmpdir") {
    it("get_tmpdir fills buffer with non-empty path") {
      char tmp[512] = {0};
      check_equal(turbo_fs_get_tmpdir(tmp, sizeof(tmp)), 0);
      check_greater((int)strlen(tmp), 0);
    }

    it("get_tmpdir with zero-size buffer returns negative") {
      char tmp[4];
      check_less(turbo_fs_get_tmpdir(tmp, 0), 0);
    }
  }
  ...
  describe("Path utilities") {
...
#ifdef _WIN32
      check_equal((int)turbo_fs_path_is_absolute("C:\\foo"), 1);
      check_equal((int)turbo_fs_path_is_absolute("\\\\srv\\share"), 1);
#else
      check_equal((int)turbo_fs_path_is_absolute("/usr/bin"), 1);
#endif
      check_equal((int)turbo_fs_path_is_absolute("relative/path"), 0);
      check_equal((int)turbo_fs_path_is_absolute(""), 0);
    }

    it("path_join produces correct joined path") {
      char result[256];
#ifdef _WIN32
      check_equal(turbo_fs_path_join(result, sizeof(result), "C:\\foo", "bar.txt"), 0);
      check_equal(result, "C:\\foo\\bar.txt");
#else
      check_equal(turbo_fs_path_join(result, sizeof(result), "/foo", "bar.txt"), 0);
    ...
    it("path_join buffer too small returns -ENAMETOOLONG") {
      char result[4];
      check_equal(turbo_fs_path_join(result, sizeof(result), "/very/long", "path.txt"),
                   -ENAMETOOLONG);
    }
    ...
    it("path_dirname extracts directory component") {
      char dir[256];
#ifdef _WIN32
      check_equal(turbo_fs_path_dirname("C:\\foo\\bar.txt", dir, sizeof(dir)), 0);
      check_equal(dir, "C:\\foo");
#else
      check_equal(turbo_fs_path_dirname("/foo/bar.txt", dir, sizeof(dir)), 0);
      check_equal(dir, "/foo");
#endif
    }
    ...
    it("path_dirname with no separator returns dot") {
      char dir[256];
      check_equal(turbo_fs_path_dirname("file.txt", dir, sizeof(dir)), 0);
      check_equal(dir, ".");
    }
    ...
    it("path_basename extracts filename component") {
      char base[256];
#ifdef _WIN32
      check_equal(turbo_fs_path_basename("C:\\foo\\bar.txt", base, sizeof(base)), 0);
#else
      check_equal(turbo_fs_path_basename("/foo/bar.txt", base, sizeof(base)), 0);
#endif
    ...
    it("path_basename with no separator returns original") {
      char base[256];
      check_equal(turbo_fs_path_basename("file.txt", base, sizeof(base)), 0);
