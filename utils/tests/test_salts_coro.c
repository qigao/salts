/**
 * @file test_salts_coro.c
 * @brief Tests for the Salts coroutine primitive.
 */
...
spec("Salts Coro Pool") {
    ...
    it("creates and destroys a generic coroutine pool") {
        ...
        salts_coro_pool_t *pool = salts_coro_pool_create(&config);

        check_not_null(pool);
        check_equal(salts_coro_pool_free_count(pool), 16);
        check_equal(salts_coro_pool_active_count(pool), 0);
        check_equal(salts_coro_pool_capacity(pool), 16);

        salts_coro_pool_destroy(pool);
    }

    it("acquires and releases pooled coroutines") {
        salts_coro_pool_config_t config = {.initial_capacity = 2, .max_capacity = 4, .stack_size = 0, .storage_size = 0};
        salts_coro_pool_t *pool = salts_coro_pool_create(&config);
        int counter = 0;
        coro_t *co1 = NULL;
        coro_t *co2 = NULL;
        ...
        check_not_null(pool);
        co1 = salts_coro_pool_acquire(pool, sched_worker, &counter);
        co2 = salts_coro_pool_acquire(pool, sched_worker, &counter);
        check_not_null(co1);
        check_not_null(co2);
        check_equal(salts_coro_pool_active_count(pool), 2);
        check_equal(salts_coro_pool_free_count(pool), 0);

        while (coro_alive(co1)) check_equal(coro_resume(co1), 0);
        while (coro_alive(co2)) check_equal(coro_resume(co2), 0);

        salts_coro_pool_release(pool, co1);
        salts_coro_pool_release(pool, co2);
        check_equal(salts_coro_pool_active_count(pool), 0);
        check_equal(salts_coro_pool_free_count(pool), 2);

        salts_coro_pool_destroy(pool);
    }

    it("respects max capacity") {
        salts_coro_pool_config_t config = {.initial_capacity = 1, .max_capacity = 2, .stack_size = 0, .storage_size = 0};
        salts_coro_pool_t *pool = salts_coro_pool_create(&config);
        int counter = 0;
        coro_t *co1 = NULL;
        coro_t *co2 = NULL;
        ...
        check_not_null(pool);
        co1 = salts_coro_pool_acquire(pool, sched_fast, &counter);
        co2 = salts_coro_pool_acquire(pool, sched_fast, &counter);
        check_not_null(co1);
        check_not_null(co2);
        check(salts_coro_pool_acquire(pool, sched_fast, &counter) == NULL);
    ...
    it("abandons a suspended frame without retaining active ownership") {
        salts_coro_pool_config_t config = {.initial_capacity = 0, .max_capacity = 1, .stack_size = 0, .storage_size = 0};
        salts_coro_pool_t *pool = salts_coro_pool_create(&config);
        int counter = 0;
        coro_t *co = NULL;

        check_not_null(pool);
        co = salts_coro_pool_acquire(pool, sched_worker, &counter);
        check_not_null(co);
        check_equal(coro_resume(co), 0);
        check_equal(coro_state(co), coro_SUSPENDED);
        check_equal(salts_coro_pool_active_count(pool), 1);
        check_equal(salts_coro_pool_retained_count(pool), 1);

        check_equal(salts_coro_pool_abandon(pool, co), 0);
        check_equal(salts_coro_pool_active_count(pool), 0);
        check_equal(salts_coro_pool_free_count(pool), 1);
        check_equal(salts_coro_pool_retained_count(pool), 0);

        co = salts_coro_pool_acquire(pool, sched_fast, &counter);
        check_not_null(co);
        check_equal(coro_resume(co), 0);
        salts_coro_pool_release(pool, co);
        salts_coro_pool_destroy(pool);
    }

    it("reuses coroutine shells without overwriting user data") {
        salts_coro_pool_config_t config = {.initial_capacity = 1, .max_capacity = 1, .stack_size = 0, .storage_size = 0};
        salts_coro_pool_t *pool = salts_coro_pool_create(&config);
        int c1 = 0;
        int c2 = 0;
        int user_data = 42;
        ...
        check_not_null(pool);
        co1 = salts_coro_pool_acquire(pool, sched_worker, &c1);
        check_not_null(co1);
        coro_set_data(co1, &user_data);
        while (coro_alive(co1)) check_equal(coro_resume(co1), 0);
        salts_coro_pool_release(pool, co1);

        co2 = salts_coro_pool_acquire(pool, sched_worker, &c2);
        check_not_null(co2);
        check(co1 == co2);
    ...
    it("preserves default storage capacity when reusing a frame") {
        ...
        check_not_null(pool);
        co = salts_coro_pool_acquire(pool, storage_pop_coro, &total);
        check_not_null(co);
    ...
    it("spawns pooled coroutines into a scheduler") {
        salts_coro_pool_config_t config = {.initial_capacity = 2, .max_capacity = 2, .stack_size = 0, .storage_size = 0};
        salts_coro_pool_t *pool = salts_coro_pool_create(&config);
        coro_scheduler_t *sched = coro_scheduler_create();
        int c1 = 0;
        int c2 = 0;
        ...
        check_not_null(pool);
        check_not_null(sched);
        check_not_null(salts_coro_spawn_pooled(sched, pool, sched_worker, &c1));
        check_not_null(salts_coro_spawn_pooled(sched, pool, sched_worker, &c2));
        check_equal(salts_coro_pool_active_count(pool), 2);
