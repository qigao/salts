...
static size_t salts_scan_mask_first_u16(unsigned int mask) {
  size_t bit = 0;
  while (bit < 16u) {
...
const char *salts_scan_char(const char *p, const char *end, char needle) {
  ...
  while ((size_t)(end - p) >= 16u) {
    ...
    if (mask != 0u) return p + salts_scan_mask_first_u16(mask);
    p += 16u;
  }
...
const char *salts_scan_to_char(const char *p, const char *end, char needle) {
  const char *hit = salts_scan_char(p, end, needle);
  return hit ? hit : end;
}

const char *salts_scan_to_any2(const char *p, const char *end, char a, char b) {
...
const char *salts_scan_to_any3(const char *p,
                               const char *end,
                               char a,
                               char b,
                               char c) {
...
const char *salts_scan_skip_sp_tab(const char *p, const char *end) {
  ...
  while ((size_t)(end - p) >= 16u) {
    ...
    return p + salts_scan_mask_first_u16((~mask) & 0xffffu);
  }
...
const char *salts_scan_skip_sp_tab_cr_lf(const char *p, const char *end) {
  const simde__m128i sp_v = simde_mm_set1_epi8(' ');
  const simde__m128i tab_v = simde_mm_set1_epi8('\t');
...
const char *salts_scan_mem(const char *haystack,
                           size_t haystack_len,
                           const char *needle,
                           size_t needle_len) {
  ...
  while (cursor < last) {
    const char *hit = salts_scan_char(cursor, last, first);
    if (!hit) return NULL;
