...
static turbo_stl_status turbo_heap_next_capacity(const turbo_heap_t *heap, size_t minimum,
                                                 size_t *out_capacity) {
    size_t capacity;
...
static turbo_stl_status turbo_heap_grow_to(turbo_heap_t *heap, size_t minimum, bool *changed) {
    void *data;
    size_t capacity, index;
    turbo_stl_status status;
    if (changed) *changed = false;
    if (!turbo_heap_valid(heap)) return TURBO_STL_INVALID_ARGUMENT;
...
static turbo_stl_status turbo_heap_prepare_copy(const turbo_heap_t *heap, const void *elem,
                                                void **out_value) {
    turbo_stl_status status;
...
static turbo_stl_status turbo_heap_initialize(turbo_heap_t *heap, const cmeta_type_desc *type,
...
turbo_stl_status turbo_heap_init_bytes(turbo_heap_t *heap, size_t elem_size, size_t elem_align,
...
turbo_stl_status turbo_heap_init(turbo_heap_t *heap, const cmeta_type_desc *element_type,
...
turbo_stl_status turbo_heap_from_array_bytes(turbo_heap_t *heap, const void *elements, size_t count,
                                             size_t elem_size, size_t elem_align, size_t limit,
                                             turbo_heap_compare_fn compare, void *context) {
    turbo_heap_t temporary = {0};
    turbo_stl_status status;
    size_t index;
    uint64_t generation;
    ...
    status = turbo_heap_init_bytes(&temporary, elem_size, elem_align, limit, compare, context);
    if (status != TURBO_STL_OK) return status;
    for (index = 0u; index < count; ++index) {
        status = turbo_heap_push(&temporary, (const unsigned char *)elements + index * elem_size);
        if (status != TURBO_STL_OK) { turbo_heap_destroy(&temporary); return status; }
    }
    generation = heap->generation + UINT64_C(1);
...
turbo_stl_status turbo_heap_from_array(turbo_heap_t *heap, const void *elements, size_t count,
    ...
    if ((count && !elements) || count > limit)
        return count > limit ? TURBO_STL_CAPACITY_EXCEEDED : TURBO_STL_INVALID_ARGUMENT;
    status = turbo_heap_init(&temporary, type, limit);
    if (status != TURBO_STL_OK) return status;
    for (index = 0u; index < count; ++index) {
        status = turbo_heap_push(&temporary, (const unsigned char *)elements + index * type->size);
        if (status != TURBO_STL_OK) { turbo_heap_destroy(&temporary); return status; }
    }
    generation = heap->generation + UINT64_C(1);
    temporary.generation = generation;
...
turbo_stl_status turbo_heap_reserve(turbo_heap_t *heap, size_t min_capacity) {
    bool changed;
    turbo_stl_status status = turbo_heap_grow_to(heap, min_capacity, &changed);
...
turbo_stl_status turbo_heap_push(turbo_heap_t *heap, const void *elem) {
    ...
    void *scratch = NULL;
    turbo_stl_status status;
    if (!turbo_heap_valid(heap) || !elem) return TURBO_STL_INVALID_ARGUMENT;
    ...
    if (status != TURBO_STL_OK) { turbo_heap_discard_prepared(heap, prepared); return status; }
    status = turbo_heap_grow_to(heap, heap->size + 1u, NULL);
    if (status != TURBO_STL_OK) { turbo_sequence_deallocate(scratch); turbo_heap_discard_prepared(heap, prepared); return status; }
    status = turbo_sequence_move_destroy(heap->element_type, heap->elem_size,
        turbo_heap_slot(heap, heap->size), prepared);
    turbo_sequence_deallocate(prepared);
    if (status != TURBO_STL_OK) { turbo_sequence_deallocate(scratch); return status; }
    ++heap->size;
