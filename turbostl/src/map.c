...
static turbostl_status map_initialize(
...
turbostl_status map_init(map_t *map,
...
static turbostl_status map_from_common(
  ...
  status = key_type != NULL
               ? map_init(&temporary, key_type, value_type, entry_limit)
...
turbostl_status map_put(map_t *map, const void *key,
                               const void *value) {
  rbtree_t *tree = map_tree(map);
...
turbostl_status map_remove(map_t *map, const void *key,
                                  void *out_value) {
  rbtree_t *tree = map_tree(map);
  rbtree_node_t *node;
  turbostl_status status;
