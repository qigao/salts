...
static turbo_stl_status turbo_map_initialize(
...
turbo_stl_status turbo_map_init(turbo_map_t *map,
...
static turbo_stl_status turbo_map_from_common(
...
turbo_stl_status turbo_map_put(turbo_map_t *map, const void *key,
                               const void *value) {
  turbo_rbtree_t *tree = turbo_map_tree(map);
...
turbo_stl_status turbo_map_remove(turbo_map_t *map, const void *key,
                                  void *out_value) {
  turbo_rbtree_t *tree = turbo_map_tree(map);
  turbo_rbtree_node_t *node;
  turbo_stl_status status;
  if (tree == NULL || key == NULL) return TURBO_STL_INVALID_ARGUMENT;
  node = turbo_rbtree_find(tree, key);
  if (node == NULL) return TURBO_STL_NOT_FOUND;
