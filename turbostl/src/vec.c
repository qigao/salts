...
static turbostl_status vec_next_capacity(const vec_t *vec, size_t minimum,
                                                size_t *out_capacity) {
    size_t capacity;
...
static turbostl_status vec_grow_to(vec_t *vec, size_t minimum, bool *changed) {
    void *data;
    size_t capacity, index;
    turbostl_status status;
    if (changed) *changed = false;
    if (!vec_valid(vec)) return TURBO_STL_INVALID_ARGUMENT;
...
static turbostl_status vec_prepare_copy(const vec_t *vec, const void *elem,
                                                void **out_value) {
    turbostl_status status;
...
static turbostl_status vec_initialize(vec_t *vec, const cmeta_type_desc *type,
...
turbostl_status vec_init_bytes(vec_t *vec, size_t elem_size, size_t elem_align,
                                      size_t element_limit) {
    return vec_initialize(vec, NULL, elem_size, elem_align, element_limit);
}

turbostl_status vec_init(vec_t *vec, const cmeta_type_desc *element_type,
                                size_t element_limit) {
    turbostl_status status;
...
turbostl_status vec_from_array_bytes(vec_t *vec, const void *elements, size_t count,
                                            size_t elem_size, size_t elem_align, size_t limit) {
    vec_t temporary = {0};
    turbostl_status status;
    size_t index;
    ...
    for (index = 0u; index < count; ++index) {
        ...
        if (status != TURBO_STL_OK) { vec_destroy(&temporary); return status; }
    }
    generation = vec->generation + UINT64_C(1);
...
turbostl_status vec_from_array(vec_t *vec, const void *elements, size_t count,
    ...
    if ((count && !elements) || count > limit)
        return count > limit ? TURBO_STL_CAPACITY_EXCEEDED : TURBO_STL_INVALID_ARGUMENT;
    status = vec_init(&temporary, type, limit);
    if (status != TURBO_STL_OK) return status;
    for (index = 0u; index < count; ++index) {
...
turbostl_status vec_reserve(vec_t *vec, size_t min_capacity) {
    bool changed;
    turbostl_status status = vec_grow_to(vec, min_capacity, &changed);
...
turbostl_status vec_push(vec_t *vec, const void *elem) {
    void *prepared = NULL;
    turbostl_status status;
    if (!vec_valid(vec) || !elem) return TURBO_STL_INVALID_ARGUMENT;
    ...
    if (status != TURBO_STL_OK) { vec_discard_prepared(vec, prepared); return status; }
    status = sequence_move_destroy(vec->element_type, vec->elem_size,
...
turbostl_status vec_insert(vec_t *vec, size_t index, const void *elem) {
    ...
    size_t cursor;
    turbostl_status status;
    if (!vec_valid(vec) || !elem || index > vec->size) return TURBO_STL_INVALID_ARGUMENT;
    if (vec->size >= vec->element_limit) return TURBO_STL_CAPACITY_EXCEEDED;
    status = vec_prepare_copy(vec, elem, &prepared);
    if (status != TURBO_STL_OK) return status;
    status = vec_grow_to(vec, vec->size + 1u, NULL);
...
turbostl_status vec_erase(vec_t *vec, size_t index, void *out_elem) {
    size_t cursor;
    turbostl_status status;
    if (!vec_valid(vec) || index >= vec->size) return TURBO_STL_INVALID_ARGUMENT;
    status = out_elem ? sequence_move_destroy(vec->element_type, vec->elem_size, out_elem,
                                                     vec_slot(vec, index))
                     : sequence_destroy_value(vec->element_type, vec_slot(vec, index));
    if (status != TURBO_STL_OK) return status;
