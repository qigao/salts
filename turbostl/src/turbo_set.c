...
turbo_stl_status turbo_set_init(turbo_set_t *set,
...
turbo_stl_status turbo_set_init_bytes(turbo_set_t *set, size_t key_size,
...
static turbo_stl_status turbo_set_from_common(
...
turbo_stl_status turbo_set_from_array(turbo_set_t *set, const void *keys,
                                      size_t count,
                                      const cmeta_type_desc *key_type,
                                      size_t element_limit) {
...
turbo_stl_status turbo_set_add(turbo_set_t *set, const void *key) {
  uint8_t present = 1u;
  if (set == NULL || key == NULL) return TURBO_STL_INVALID_ARGUMENT;
  if (turbo_map_contains(&set->map, key)) return TURBO_STL_OK;
...
turbo_stl_status turbo_set_remove(turbo_set_t *set, const void *key) {
