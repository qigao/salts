...
suite("TurboSTL sequences") {
    it("keeps raw vector values bounded and zero fills growth") {
        ...
        check_equal(vec_init_bytes(&vec, sizeof(value), _Alignof(int), 3u),
                    TURBO_STL_OK);
        check_equal(vec_push(&vec, &value), TURBO_STL_OK);
        check_equal(vec_resize(&vec, 3u), TURBO_STL_OK);
        ...
        check_equal(vec_push(&vec, &value), TURBO_STL_CAPACITY_EXCEEDED);
        vec_destroy(&vec);
    }
    ...
    it("rejects capacity byte overflow without changing the vector") {
        ...
        check_equal(vec_reserve(&vec, SIZE_MAX), TURBO_STL_CAPACITY_EXCEEDED);
    ...
    it("preserves deque order and rejects a value past its limit") {
        ...
        check_equal(deque_init_bytes(&deque, sizeof(one), _Alignof(int), 3u),
                    TURBO_STL_OK);
        check_equal(deque_push_back(&deque, &two), TURBO_STL_OK);
        check_equal(deque_push_front(&deque, &one), TURBO_STL_OK);
        check_equal(deque_push_back(&deque, &three), TURBO_STL_OK);
        check_equal(deque_pop_front(&deque, &out), TURBO_STL_OK);
        check_equal(out, 1);
        check_equal(deque_push_back(&deque, &one), TURBO_STL_OK);
        check_equal(deque_push_back(&deque, &one), TURBO_STL_CAPACITY_EXCEEDED);
    ...
    it("uses raw heap comparator while preserving the heap invariant") {
        ...
        for (index = 0u; index < sizeof(values) / sizeof(values[0]); ++index)
            check_equal(heap_push(&heap, &values[index]), TURBO_STL_OK);
    ...
    it("orders records larger than the former inline swap buffer") {
        ...
        for (index = 0u; index < 5u; ++index) {
            ...
            check_equal(heap_push(&heap, &item), TURBO_STL_OK);
        }
        for (index = 0u; index < 5u; ++index) {
    ...
    it("honors explicit over-alignment") {
        ...
        check_equal(vec_init_bytes(&vec, sizeof(byte), 64u, 1u), TURBO_STL_OK);
        check_equal(vec_push(&vec, &byte), TURBO_STL_OK);
    ...
    it("leaves destroyed byte handles unchanged when from-array exceeds the limit") {
        ...
        check_equal(deque_init_bytes(&deque, sizeof(int), _Alignof(int), 1u), TURBO_STL_OK);
        deque_destroy(&deque);
        deque_before = deque;
    ...
    it("does not write removal output on failure") {
        ...
        check_equal(vec_init_bytes(&vec, sizeof(int), _Alignof(int), 1u), TURBO_STL_OK);
        check_equal(vec_pop(&vec, &output), TURBO_STL_EMPTY);
        check_equal(output, 91);
        check_equal(vec_erase(&vec, 0u, &output), TURBO_STL_INVALID_ARGUMENT);
        check_equal(output, 91);
        check_equal(vec_swap_remove(&vec, 0u, &output), TURBO_STL_INVALID_ARGUMENT);
        check_equal(output, 91);
        vec_destroy(&vec);

        check_equal(deque_init_bytes(&deque, sizeof(int), _Alignof(int), 1u), TURBO_STL_OK);
        check_equal(deque_pop_back(&deque, &output), TURBO_STL_EMPTY);
        check_equal(output, 91);
        check_equal(deque_pop_front(&deque, &output), TURBO_STL_EMPTY);
        check_equal(output, 91);
        deque_destroy(&deque);

        check_equal(heap_init_bytes(&heap, sizeof(int), _Alignof(int), 1u, int_compare, NULL),
                    TURBO_STL_OK);
        check_equal(heap_pop(&heap, &output), TURBO_STL_EMPTY);
        check_equal(output, 91);
        heap_destroy(&heap);
    }
}
