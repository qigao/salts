...
suite("TurboSTL hash ownership") {
    it("keeps HashSet independent with collision and duplicate semantics") {
        ...
        key = 1;
        check_equal(hash_set_remove(&set, &key), TURBO_STL_OK);
    ...
    it("keeps collisions tombstones and rehash inside bucket arrays") {
        ...
        for (key = 0; key < 4; ++key)
            check_equal(hash_map_remove(&map, &key, NULL), TURBO_STL_OK);
        check_equal(map.tombstones, (size_t)4u);
        capacity = hash_map_capacity(&map);
        generation = hash_map_generation(&map);
        check_equal(hash_map_reserve(&map, 2u), TURBO_STL_OK);
    ...
    it("requires explicit raw callbacks and typed key traits") {
                    ...
                    TURBO_STL_OK);
        {
            ...
            check_equal(hash_map_put(&map, &key, &value), TURBO_STL_CAPACITY_EXCEEDED);
    ...
    it("enforces live entry limits and honors raw alignment") {
        ...
        check_equal(hash_map_put(&map, &two, &value), TURBO_STL_CAPACITY_EXCEEDED);
    ...
    it("keeps replacement transactional including self aliases") {
        ...
        check_equal(hash_map_init(&map, &counted_type, &counted_type, 2u), TURBO_STL_OK);
        check_equal(hash_map_put(&map, &key, &first), TURBO_STL_OK);
        generation = map.generation;
        fail_copy_on = copies + 1u;
        check_equal(hash_map_put(&map, &key, &second), TURBO_STL_OUT_OF_MEMORY);
        stored = (counted_value *)hash_map_get(&map, &key);
        check_equal(*stored->value, 7);
        check_equal(map.generation, generation);
        fail_copy_on = 0u;
        check_equal(hash_map_put(&map, &key, stored), TURBO_STL_OK);
    ...
    it("moves removals without writing output for a miss and balances clear destroy") {
        ...
        check_equal(hash_map_init(&map, &counted_type, &counted_type, 2u), TURBO_STL_OK);
        check_equal(hash_map_put(&map, &key, &value), TURBO_STL_OK);
        check_equal(hash_map_remove(&map, &missing, &unchanged), TURBO_STL_NOT_FOUND);
        check_equal(*unchanged.value, 77);
        check_equal(hash_map_remove(&map, &key, &out), TURBO_STL_OK);
    ...
    it("commits typed from arrays only after every copy succeeds") {
        ...
        before = map;
        fail_copy_on = 3u;
        check_equal(hash_map_from_arrays(&map, keys, values, 2u, &counted_type,
    ...
    it("keeps lifecycle generations transactional around reserve and tombstones") {
        ...
        generation = map.generation;
        check_equal(hash_map_reserve(&map, 1u), TURBO_STL_OK);
    ...
    it("copies an aliased value before an insertion can rehash storage") {
        ...
        check_equal(hash_map_put(&map, &alias_key, aliased_value), TURBO_STL_OK);
    ...
    it("keeps ordered multimap duplicates under one element limit") {
        ...
        check_equal(multimap_init_bytes(
                    ...
                    TURBO_STL_OK);
        check_equal(multimap_put(&map, &one, &first), TURBO_STL_OK);
        check_equal(multimap_put(&map, &one, &second), TURBO_STL_OK);
    ...
    it("counts typed set ownership and accepts duplicate arrays by live entry limit") {
        ...
        reset_counts();
        key = counted_make(1);
        check_equal(hash_set_init(&set, &counted_type, 1u), TURBO_STL_OK);
        check_equal(hash_set_add(&set, &key), TURBO_STL_OK);
        generation = hash_set_generation(&set);
    ...
    it("balances typed clear and NULL removal ownership") {
        ...
        check_equal(hash_map_init(&map, &counted_type, &counted_type, 2u), TURBO_STL_OK);
        check_equal(hash_map_put(&map, &keys[0], &values[0]), TURBO_STL_OK);
        check_equal(hash_map_put(&map, &keys[1], &values[1]), TURBO_STL_OK);
        ...
        check_equal(hash_map_init(&map, &counted_type, &counted_type, 1u), TURBO_STL_OK);
        check_equal(hash_map_put(&map, &keys[0], &values[0]), TURBO_STL_OK);
        check_equal(hash_map_remove(&map, &keys[0], NULL), TURBO_STL_OK);
    ...
    it("moves and destroys typed entries during rehash") {
        ...
        check_equal(hash_map_init(&map, &counted_type, &counted_type, 12u), TURBO_STL_OK);
        for (index = 0u; index < 12u; ++index)
            check_equal(hash_map_put(&map, &keys[index], &values[index]), TURBO_STL_OK);
