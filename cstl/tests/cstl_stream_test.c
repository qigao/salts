...
suite("CSTL CFlow Stream") {
    ...
    it("exposes prefixed scalar and visiting terminal conveniences") {
        ...
        check_equal(count, (size_t)2u);
        {
            cstl_status_result result =
                cstl_stream_count_result(&pipeline, &count);
            check_true(cstl_status_result_is_ok(result));
            check_equal(result.status, CFLOW_STATUS_OK);
            check_true(strcmp(
                cstl_status_result_message(result), "ok") == 0);
            check_equal(count, (size_t)2u);
        }
        check_true(cstl_stream_any_match(
            &pipeline, stream_keep_even, &matched, &error));
        check_true(matched);
        check_true(cstl_stream_all_match(
            &pipeline, stream_keep_even, &matched, &error));
        check_true(matched);
        ...
        check_true(cmeta_type_equal(
            cstl_find_result_type(&found), &cmeta_type_int));
        check_equal(*(const int *)cstl_find_result_value(&found), 4);
        check_true(cstl_stream_for_each(
    ...
    it("reports the CSTL sorted hard bound structurally") {
        ...
        result = cstl_stream_count_result(&pipeline, &count);
    ...
    it("reports the CSTL distinct hard bound structurally") {
