...
suite("CSTL generic type identities") {
    it("exposes canonical generic applications for every typed container kind") {
        ...
        check_equal(sizeof(canonical_constructors) /
                    ...
                    (size_t)13u);
        CHECK_UNARY_APPLICATION(vec, "cstl.Vec");
        CHECK_UNARY_APPLICATION(deque, "cstl.Deque");
        CHECK_UNARY_APPLICATION(list, "cstl.List");
        CHECK_UNARY_APPLICATION(stack, "cstl.Stack");
        CHECK_UNARY_APPLICATION(queue, "cstl.Queue");
        CHECK_UNARY_APPLICATION(heap, "cstl.Heap");
        CHECK_UNARY_APPLICATION(set, "cstl.Set");
        CHECK_UNARY_APPLICATION(hash_set, "cstl.HashSet");
        CHECK_BINARY_APPLICATION(hash_map, "cstl.HashMap");
        CHECK_BINARY_APPLICATION(map, "cstl.Map");
        CHECK_BINARY_APPLICATION(multimap, "cstl.MultiMap");
        CHECK_BINARY_APPLICATION(btree, "cstl.BTree");
        CHECK_BINARY_APPLICATION(bplus_tree, "cstl.BPlusTree");
    }
