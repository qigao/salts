...
spec("CSTL typed C++ public header") {
  ...
  it("exposes canonical generic metadata without C-only declaration macros") {
    ...
    check_equal(std::strcmp(stl_vec_generic_desc.stable_id, "cstl.Vec"), 0);
    ...
    check_equal(std::strcmp(stl_map_generic_desc.stable_id, "cstl.Map"), 0);
  }
