...
enum { SALTS_TEST_CONTAINER_KIND_COUNT = 0 Replay(SALTS_STL_KIND_SCHEMA, COUNT_CONTAINER_KIND) };
#undef COUNT_CONTAINER_KIND

#define DECLARE_CONTAINER_KIND_ARITY(kind,arity,family,raw,prefix,methods,accept,key_at,value_at,range_flags,key_flags,value_flags,entry_flags) SALTS_TEST_ARITY_##kind = arity,
enum { Replay(SALTS_STL_KIND_SCHEMA, DECLARE_CONTAINER_KIND_ARITY) };
#undef DECLARE_CONTAINER_KIND_ARITY

_Static_assert(SALTS_TEST_CONTAINER_KIND_COUNT == 13,
               "SALTS_STL_KIND_SCHEMA_COUNT_MISMATCH");
_Static_assert(SALTS_TEST_ARITY_Vec == 1 && SALTS_TEST_ARITY_HashMap == 2 &&
