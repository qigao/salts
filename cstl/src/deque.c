...
#define SALTS_DEQUE_MIN_CAPACITY 8u
...
static stl_status deque_next_capacity(const deque_t *deque, size_t minimum,
                                                  size_t *out_capacity) {
    size_t capacity;
    if (minimum > deque->element_limit) return STL_CAPACITY_EXCEEDED;
    capacity = deque->capacity ? deque->capacity : SALTS_DEQUE_MIN_CAPACITY;
...
static stl_status deque_grow_to(deque_t *deque, size_t minimum, bool *changed) {
    void *data;
    size_t capacity, index;
    stl_status status;
    if (changed) *changed = false;
    if (!deque_valid(deque)) return STL_INVALID_ARGUMENT;
    if (minimum <= deque->capacity) return STL_OK;
...
static stl_status deque_prepare_copy(const deque_t *deque, const void *elem,
                                                  void **out_value) {
    stl_status status;
...
static stl_status deque_initialize(deque_t *deque, const cmeta_type_desc *type,
                                               size_t size, size_t align, size_t limit) {
    stl_status status;
    size_t stride;
    uint64_t generation;
    if (!deque) return STL_INVALID_ARGUMENT;
    if (deque->initialized) return STL_INVALID_ARGUMENT;
...
stl_status deque_raw_init(deque_t *deque, const cmeta_type_desc *element_type,
                                  size_t element_limit) {
    stl_status status;
    if (!deque || deque->initialized) return STL_INVALID_ARGUMENT;
...
stl_status deque_from_array_bytes(deque_t *deque, const void *elements,
    ...
    for (index = 0u; index < count; ++index) {
        ...
        if (status != STL_OK) { deque_raw_destroy_storage(&temporary); return status; }
    }
    generation = deque->generation + UINT64_C(1);
...
stl_status deque_raw_from_array(deque_t *deque, const void *elements, size_t count,
    ...
    if ((count && !elements) || count > limit)
        return count > limit ? STL_CAPACITY_EXCEEDED : STL_INVALID_ARGUMENT;
    status = deque_raw_init(&temporary, type, limit);
    if (status != STL_OK) return status;
    for (index = 0u; index < count; ++index) {
...
stl_status deque_reserve(deque_t *deque, size_t min_capacity) {
    bool changed;
    stl_status status = deque_grow_to(deque, min_capacity, &changed);
    if (status == STL_OK && changed) ++deque->generation;
    return status;
}
...
stl_status deque_pop_back(deque_t *deque, void *out_elem) {
    ...
    size_t physical;
    if (!deque_valid(deque)) return STL_INVALID_ARGUMENT;
    if (!deque->size) return STL_EMPTY;
...
stl_status deque_set(deque_t *deque, size_t index, const void *elem) {
    ...
    stl_status status;
    unsigned char *slot;
    if (!deque_valid(deque) || !elem || index >= deque->size) return STL_INVALID_ARGUMENT;
    status = deque_prepare_copy(deque, elem, &prepared);
    if (status != STL_OK) return status;
