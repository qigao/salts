...
suite("regex and contains benchmarks") {
  ...
  bench("SIMDe first-byte scanning") {
    benchmark_bytes("scalar 64KiB first-byte scan", 1000, SCAN_BYTES) {
      check_null(re_scan_first_byte_scalar(buf, buf + SCAN_BYTES, (unsigned char)'z'));
    }
    benchmark_bytes("SIMDe 64KiB first-byte scan", 1000, SCAN_BYTES) {
      check_null(re_scan_first_byte_simde(buf, buf + SCAN_BYTES, (unsigned char)'z'));
    }
  }
