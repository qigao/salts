...
typedef enum cxml_failure_kind {
    CXML_FAILURE_FATAL = 0,
    CXML_FAILURE_ALLOCATION
} cxml_failure_kind;

typedef void (*cxml_failure_handler)(cxml_failure_kind kind,
                                     const char *message,
                                     void *user);

void cxml_set_failure_handler(cxml_failure_handler handler, void *user);

/* Private fault-injection hook used by the TurboUtils adapter tests. */
...
#define FREE(ptr)                               (_cxml_free(ptr))
