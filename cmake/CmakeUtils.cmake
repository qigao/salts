...
function(_cmake_resolve_target_folder output_var requested_folder)
    if(requested_folder)
        set(target_folder "${requested_folder}")
    else()
        file(RELATIVE_PATH target_folder "${PROJECT_SOURCE_DIR}" "${CMAKE_CURRENT_SOURCE_DIR}")
        if(NOT target_folder OR target_folder STREQUAL ".")
            set(target_folder "root")
        endif()
        if(ARGN)
            cmake_path(APPEND target_folder ${ARGN})
        endif()
    endif()

    set(${output_var} "${target_folder}" PARENT_SCOPE)
endfunction()
...
function(cmake_config_target target_name)
    ...
    _cmake_resolve_target_folder(target_folder "${ARG_FOLDER}")
...
function(cmake_add_grammar TARGET_NAME)
  ...
  _cmake_resolve_target_folder(target_folder "${ARG_FOLDER}" grammar)
  ...
  if(ARG_LEXER_RE)
    ...
    set_target_properties(${LEXER_TARGET} PROPERTIES FOLDER "${target_folder}")
  ...
  if(ARG_GRAMMAR_Y)
    ...
    set_target_properties(${GRAMMAR_TARGET} PROPERTIES FOLDER "${target_folder}")
