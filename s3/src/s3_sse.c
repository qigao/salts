...
static int s3_sse_base64(const void *data, size_t size, size_t max_header_bytes, char **out_value) {
  ...
  if (!result.ok) return result.error == TN_BASE64_ERR_NO_MEMORY ? TURBO_ENOMEM : TURBO_EINVAL;
...
static int s3_sse_customer_headers(const s3_sse_options *options, size_t max_header_bytes,
  ...
  if (status == TURBO_OK && (EVP_Digest(options->customer_key, options->customer_key_size, digest,
...
int s3_sse_headers_build(const s3_sse_options *options, int for_write, size_t max_header_bytes,
  ...
  int status;
  if (out_headers == NULL) return TURBO_EINVAL;
  *out_headers = (s3_sse_headers){0};
  if (options == NULL) return TURBO_OK;
  if (options->size != sizeof(*options) || max_header_bytes == 0u) return TURBO_EINVAL;
  switch (options->mode) {
  case S3_SSE_NONE:
    return s3_sse_has_kms_fields(options) || s3_sse_has_customer_fields(options) ? TURBO_EINVAL
                                                                                 : TURBO_OK;
  case S3_SSE_S3:
  ...
  case S3_SSE_KMS:
    ...
    if (options->kms_context == NULL) return TURBO_OK;
