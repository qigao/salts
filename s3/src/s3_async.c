...
static void s3_async_complete(void *user, chttp_request request,
  ...
  if (http_error != NULL) {
    ...
    error = s3_async_error(TURBO_EPROTO, 0, "s3-response");
    error_pointer = &error;
  } else {
    ...
    if (http_response->status_code < 200u || http_response->status_code >= 300u) {
      ...
      error = status == TURBO_OK ? s3_async_error(TURBO_EPROTO, 0, "s3-service")
...
int s3_async_client_destroy(s3_async_client *client) {
  ...
  if (impl->active_requests != 0u || impl->callback_active) return TURBO_EBUSY;
