...
static int s3_multipart_part_sse_headers_init(const s3_client_base *client,
  ...
  if (status == TURBO_OK && options != NULL && options->sse != NULL &&
...
static int s3_multipart_upload_id_parse(const s3_client_impl *client, const s3_response *response,
  ...
  if (status == TURBO_OK) {
    ...
    status = upload_id.impl != NULL ? s3_xml_text_dup(upload_id, out_upload_id) : TURBO_EPROTO;
    if (status == TURBO_OK && (*out_upload_id)[0] == '\0') {
      free(*out_upload_id);
      *out_upload_id = NULL;
      status = TURBO_EPROTO;
    }
...
int s3_multipart_initiate(s3_client *client, const char *bucket, const char *key,
  ...
  *out_error = (s3_error){0};
  if (client_impl == NULL ||
    ...
    return s3_multipart_error(out_error, TURBO_EINVAL, "s3-multipart-initiate");
  ...
  if (status == TURBO_OK &&
      s3_checked_multiply(client_impl->base.config.max_multipart_parts,
                          sizeof(s3_multipart_part_internal), &parts_bytes) != TURBO_OK)
    status = TURBO_ERANGE;
  if (status == TURBO_OK) {
    upload = (s3_multipart_impl *)calloc(1u, sizeof(*upload));
    if (upload == NULL) status = TURBO_ENOMEM;
  }
  if (status == TURBO_OK) {
  ...
  if (status == TURBO_OK) status = s3_request(client, &request, out_response, out_error);
  if (status == TURBO_OK)
  ...
  if (status == TURBO_OK) {
    ...
    upload = NULL;
  } else if (out_error->status == TURBO_OK) {
...
static int s3_multipart_active(s3_multipart *upload, s3_multipart_impl **out_impl,
  ...
  if (impl == NULL || impl->state != S3_MULTIPART_ACTIVE) {
    s3_multipart_error(out_error, TURBO_EINVAL, stage);
    return TURBO_EINVAL;
  }
  *out_impl = impl;
  return TURBO_OK;
...
int s3_multipart_upload_part(s3_client *client, s3_multipart *upload, uint32_t part_number,
  ...
  *out_error = (s3_error){0};
  ...
  if (client == NULL || client->impl == NULL || part_number == 0u ||
    ...
    return s3_multipart_error(out_error, TURBO_EINVAL, "s3-multipart-part");
  text_size = snprintf(part_text, sizeof(part_text), "%u", (unsigned int)part_number);
  if (text_size <= 0 || (size_t)text_size >= sizeof(part_text))
    return s3_multipart_error(out_error, TURBO_ERANGE, "s3-multipart-part");
  ...
  if (status != TURBO_OK) return s3_multipart_error(out_error, status, "s3-multipart-part-hash");
  request = (s3_request_options){.size = sizeof(request),
  ...
  if (etag == NULL || etag[0] == '\0')
    return s3_multipart_error(out_error, TURBO_EPROTO, "s3-multipart-etag");
  owned_etag = s3_multipart_text_dup(etag);
  if (owned_etag == NULL) return s3_multipart_error(out_error, TURBO_ENOMEM, "s3-multipart-etag");
...
static int s3_multipart_append_xml_text(s3_text_builder *builder, const char *text) {
  size_t index;
  int status = TURBO_OK;
  for (index = 0u; text[index] != '\0' && status == TURBO_OK; ++index) {
    switch (text[index]) {
    case '&':
...
static int s3_multipart_append_element(s3_text_builder *builder, const char *name,
                                       const char *value) {
  int status;
  if (builder == NULL || name == NULL || value == NULL) return TURBO_EINVAL;
  status = s3_text_builder_append(builder, "<", 1u);
  if (status == TURBO_OK) status = s3_text_builder_append_cstr(builder, name);
  if (status == TURBO_OK) status = s3_text_builder_append(builder, ">", 1u);
  if (status == TURBO_OK) status = s3_multipart_append_xml_text(builder, value);
  if (status == TURBO_OK) status = s3_text_builder_append(builder, "</", 2u);
  if (status == TURBO_OK) status = s3_text_builder_append_cstr(builder, name);
  if (status == TURBO_OK) status = s3_text_builder_append(builder, ">", 1u);
  return status;
}
...
static int s3_multipart_capacity_add_text(size_t *capacity, const char *text) {
  ...
  if (capacity == NULL || text == NULL || strlen(text) > SIZE_MAX / 6u) return TURBO_ERANGE;
...
static int s3_multipart_checkpoint_capacity(const s3_client_base *client,
  ...
  int status = TURBO_OK;
  for (index = 0u; index < sizeof(fields) / sizeof(fields[0]) && status == TURBO_OK; ++index)
    status = s3_multipart_capacity_add_text(&capacity, fields[index]);
  for (index = 0u; index < upload->part_capacity && status == TURBO_OK; ++index) {
  ...
  if (status == TURBO_OK && capacity > client->config.max_xml_bytes) status = TURBO_EMSGSIZE;
  if (status == TURBO_OK) *out_capacity = capacity;
  return status;
}

static int s3_multipart_checkpoint_xml(const s3_client_base *client,
                                       const s3_multipart_impl *upload, const char *source_path,
                                       const turbo_fs_stat_t *file_stat, size_t part_size,
                                       tstr *out_xml) {
  ...
  if (status == TURBO_OK) status = s3_text_builder_init(&builder, capacity);
  if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, "<S3MultipartResume>");
  if (status == TURBO_OK)
  ...
  if (status == TURBO_OK) status = s3_multipart_append_element(&builder, "Region", client->region);
  if (status == TURBO_OK) {
  ...
  if (status == TURBO_OK) status = s3_multipart_append_element(&builder, "Bucket", upload->bucket);
  if (status == TURBO_OK) status = s3_multipart_append_element(&builder, "Key", upload->key);
  if (status == TURBO_OK) status = s3_multipart_append_element(&builder, "SourcePath", source_path);
  if (status == TURBO_OK) {
  ...
  if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, "<Parts>");
  for (index = 0u; index < upload->part_capacity && status == TURBO_OK; ++index) {
    if (upload->parts[index].etag == NULL) continue;
    ...
    if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, "</Part>");
  }
  if (status == TURBO_OK)
    status = s3_text_builder_append_cstr(&builder, "</Parts></S3MultipartResume>");
  if (status == TURBO_OK) *out_xml = s3_text_builder_release(&builder);
...
static int s3_multipart_checkpoint_temp_path(const char *checkpoint_path, char **out_path) {
  ...
  char *path;
  int status;
  if (checkpoint_path == NULL || out_path == NULL || *out_path != NULL) return TURBO_EINVAL;
  status = turbo_uuid_v4_generate(&uuid);
  if (status == TURBO_OK) status = turbo_uuid_format(&uuid, uuid_text, sizeof(uuid_text));
  if (status != TURBO_OK) return status;
  base_size = strlen(checkpoint_path);
  if (base_size >
      SIZE_MAX - (sizeof(prefix) - 1u) - TURBO_UUID_STRING_LENGTH - (sizeof(suffix) - 1u))
    return TURBO_ERANGE;
  total_size = base_size + sizeof(prefix) - 1u + TURBO_UUID_STRING_LENGTH + sizeof(suffix) - 1u;
  ...
  memcpy(path + base_size + sizeof(prefix) - 1u, uuid_text, TURBO_UUID_STRING_LENGTH);
  memcpy(path + total_size - (sizeof(suffix) - 1u), suffix, sizeof(suffix));
  *out_path = path;
  return TURBO_OK;
}

static int s3_multipart_checkpoint_write_all(turbo_file_t file, const char *data, size_t size) {
  size_t offset = 0u;
  while (offset < size) {
    const size_t remaining = size - offset;
    const size_t chunk = remaining < (size_t)INT_MAX ? remaining : (size_t)INT_MAX;
    const int written = turbo_fs_write(file, data + offset, chunk);
...
static int s3_multipart_checkpoint_store(const s3_client_base *client,
  ...
  if (status == TURBO_OK) {
    file = turbo_fs_open(temporary_path, TURBO_FS_O_WRONLY | TURBO_FS_O_CREAT | TURBO_FS_O_TRUNC,
                         S3_MULTIPART_CHECKPOINT_MODE);
    if (file == TURBO_INVALID_FILE) status = TURBO_EIO;
  }
  if (status == TURBO_OK) status = s3_multipart_checkpoint_write_all(file, xml, tstr_len(xml));
  if (status == TURBO_OK) status = turbo_fs_fsync(file);
  if (file != TURBO_INVALID_FILE) close_status = turbo_fs_close(file);
  if (status == TURBO_OK) status = close_status;
  if (status == TURBO_OK) status = turbo_fs_rename(temporary_path, checkpoint_path);
  if (status != TURBO_OK && temporary_path != NULL) (void)turbo_fs_unlink(temporary_path);
  free(temporary_path);
  tstr_free(xml);
  return status == TURBO_OK ? TURBO_OK : TURBO_EIO;
}

static int s3_multipart_parse_u64(const char *text, uint64_t *out_value) {
  uint64_t value = 0u;
  size_t index;
  if (text == NULL || text[0] == '\0' || out_value == NULL) return TURBO_EPROTO;
...
static int s3_multipart_child_text(turbo_xml_node parent, const char *name, char **out_text) {
  const turbo_xml_node child = s3_xml_child(parent, name);
  if (child.impl == NULL || out_text == NULL || *out_text != NULL) return TURBO_EPROTO;
  return s3_xml_text_dup(child, out_text);
}

static int s3_multipart_checkpoint_part_parse(s3_multipart_impl *upload, turbo_xml_node node) {
  ...
  if (status == TURBO_OK) status = s3_multipart_child_text(node, "Size", &size_text);
  if (status == TURBO_OK) status = s3_multipart_child_text(node, "ETag", &etag);
  if (status == TURBO_OK) status = s3_multipart_parse_u64(number_text, &number);
  if (status == TURBO_OK) status = s3_multipart_parse_u64(size_text, &size);
  if (status == TURBO_OK && (number == 0u || number > upload->part_capacity || size > SIZE_MAX ||
...
static int s3_multipart_checkpoint_parts_parse(s3_multipart_impl *upload, turbo_xml_node parts) {
  size_t index;
  int status = TURBO_OK;
  if (parts.impl == NULL) return TURBO_EPROTO;
  for (index = 0u; index < turbo_xml_node_child_count(parts) && status == TURBO_OK; ++index) {
    const turbo_xml_node child = turbo_xml_node_child_at(parts, index);
    if (turbo_xml_node_type(child) == TURBO_XML_ELEMENT) {
      if (!s3_xml_node_name_equal(child, "Part")) return TURBO_EPROTO;
...
static int s3_multipart_checkpoint_identity(const s3_client_base *client, turbo_xml_node root,
  ...
  size_t index;
  int status = TURBO_OK;
  for (index = 0u; index < sizeof(names) / sizeof(names[0]) && status == TURBO_OK; ++index)
    status = s3_multipart_child_text(root, names[index], &values[index]);
  if (status == TURBO_OK && strcmp(values[0], s3_multipart_checkpoint_version) != 0)
    status = TURBO_EPROTO;
  if (status == TURBO_OK && strcmp(values[1], client->connection_uri) != 0) status = TURBO_EINVAL;
  if (status == TURBO_OK && strcmp(values[2], client->authority) != 0) status = TURBO_EINVAL;
  if (status == TURBO_OK && strcmp(values[3], client->region) != 0) status = TURBO_EINVAL;
  (void)snprintf(expected, sizeof(expected), "%u", (unsigned int)client->config.addressing_style);
  if (status == TURBO_OK && strcmp(values[4], expected) != 0) status = TURBO_EINVAL;
  if (status == TURBO_OK && strcmp(values[5], bucket) != 0) status = TURBO_EINVAL;
  if (status == TURBO_OK && strcmp(values[6], key) != 0) status = TURBO_EINVAL;
  if (status == TURBO_OK && strcmp(values[7], source_path) != 0) status = TURBO_EINVAL;
  (void)snprintf(expected, sizeof(expected), "%llu", (unsigned long long)file_stat->size);
  if (status == TURBO_OK && strcmp(values[8], expected) != 0) status = TURBO_EINVAL;
  (void)snprintf(expected, sizeof(expected), "%llu", (unsigned long long)file_stat->mtime);
  if (status == TURBO_OK && strcmp(values[9], expected) != 0) status = TURBO_EINVAL;
  (void)snprintf(expected, sizeof(expected), "%llu", (unsigned long long)file_stat->ctime);
  if (status == TURBO_OK && strcmp(values[10], expected) != 0) status = TURBO_EINVAL;
  (void)snprintf(expected, sizeof(expected), "%llu", (unsigned long long)part_size);
  if (status == TURBO_OK && strcmp(values[11], expected) != 0) status = TURBO_EINVAL;
  if (status == TURBO_OK && strcmp(values[12], s3_multipart_customer_key_md5(upload)) != 0)
    status = TURBO_EINVAL;
  if (status == TURBO_OK && values[13][0] == '\0') status = TURBO_EPROTO;
  if (status == TURBO_OK) {
...
static int s3_multipart_checkpoint_load(const s3_client_base *client, const char *bucket,
  ...
  turbo_fs_stat_t checkpoint_stat = {0};
  turbo_fs_buf_t file = {0};
  turbo_xml_document document = {0};
  turbo_xml_node root = {0};
  turbo_xml_node parts;
  s3_multipart_impl *upload = NULL;
  size_t parts_bytes = 0u;
  int status;
  status = turbo_fs_stat(checkpoint_path, &checkpoint_stat);
  if (status != TURBO_OK) return status == -ENOENT ? TURBO_ENOENT : TURBO_EIO;
  if (!checkpoint_stat.is_file) return TURBO_EINVAL;
  if (checkpoint_stat.size > client->config.max_xml_bytes) return TURBO_EMSGSIZE;
  status = turbo_fs_read_file(checkpoint_path, &file);
  ...
  if (status == TURBO_OK) status = s3_multipart_part_sse_headers_init(client, options, upload);
  if (status == TURBO_OK)
...
static int s3_multipart_completion_xml(const s3_client_impl *client,
  ...
  for (index = 0u; index < upload->part_capacity && status == TURBO_OK; ++index) {
    size_t etag_limit;
    if (upload->parts[index].etag == NULL) continue;
    if (strlen(upload->parts[index].etag) > (SIZE_MAX / 6u)) return TURBO_ERANGE;
    ...
    if (s3_checked_add(capacity,
                       ...
                       &capacity) != TURBO_OK ||
        s3_checked_add(capacity, etag_limit, &capacity) != TURBO_OK ||
        capacity > client->base.config.max_xml_bytes)
      return TURBO_EMSGSIZE;
  }
  status = s3_text_builder_init(&builder, capacity);
  if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, prefix);
  for (index = 0u; index < upload->part_capacity && status == TURBO_OK; ++index) {
    char part_number[16];
    int written;
    if (upload->parts[index].etag == NULL) continue;
    ...
    if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, part_number);
    if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, part_middle);
    if (status == TURBO_OK)
      status = s3_multipart_append_xml_text(&builder, upload->parts[index].etag);
    if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, part_suffix);
  }
  if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, suffix);
  if (status == TURBO_OK) *out_xml = s3_text_builder_release(&builder);
  s3_text_builder_destroy(&builder);
  return status;
}
...
static int s3_multipart_validate_completion(const s3_multipart_impl *upload) {
  size_t highest;
  size_t index;
  if (upload->part_count == 0u) return TURBO_EINVAL;
  highest = upload->part_capacity;
  while (highest != 0u && upload->parts[highest - 1u].etag == NULL)
    --highest;
  if (highest == 0u || highest != upload->part_count) return TURBO_EINVAL;
...
static int s3_multipart_complete_response_validate(const s3_client_impl *client,
  ...
  if (response->http.body == NULL || response->http.body_size == 0u)
    return s3_multipart_error(out_error, TURBO_EPROTO, "s3-multipart-complete-parse");
  ...
  if (status != TURBO_OK)
    return s3_multipart_error(out_error, status, "s3-multipart-complete-parse");
  return s3_multipart_error(out_error, TURBO_EPROTO, "s3-multipart-complete-service");
}
...
int s3_multipart_complete(s3_client *client, s3_multipart *upload, s3_response *out_response,
  ...
  *out_error = (s3_error){0};
  ...
  if (status != TURBO_OK) return status;
  if (client_impl == NULL)
    return s3_multipart_error(out_error, TURBO_EINVAL, "s3-multipart-complete");
  status = s3_multipart_validate_completion(impl);
  if (status == TURBO_OK) status = s3_multipart_completion_xml(client_impl, impl, &xml);
  if (status != TURBO_OK) return s3_multipart_error(out_error, status, "s3-multipart-complete");
  ...
  if (status == TURBO_OK) impl->state = S3_MULTIPART_COMPLETED;
  tstr_free(xml);
  return status;
}
...
int s3_multipart_abort(s3_client *client, s3_multipart *upload, s3_response *out_response,
  ...
  *out_error = (s3_error){0};
  ...
  request = (s3_request_options){.size = sizeof(request),
                                 ...
                                 .query_count = 1u};
  status = s3_request(client, &request, out_response, out_error);
  if (status == TURBO_OK) impl->state = S3_MULTIPART_ABORTED;
  return status;
}

static int s3_multipart_file_read_part(turbo_file_t file, unsigned char *buffer, size_t size,
                                       uint64_t offset, s3_error *out_error) {
  size_t consumed = 0u;
  if (file == TURBO_INVALID_FILE || buffer == NULL || size == 0u || offset > INT64_MAX)
    return s3_multipart_error(out_error, TURBO_EINVAL, "multipart-file-read");
  while (consumed < size) {
    if ((uint64_t)consumed > (uint64_t)INT64_MAX - offset)
      return s3_multipart_error(out_error, TURBO_ERANGE, "multipart-file-read");
    const int read_size = turbo_fs_pread(file, (char *)buffer + consumed, size - consumed,
                                         (int64_t)(offset + consumed));
    if (read_size <= 0) return s3_multipart_error(out_error, TURBO_EIO, "multipart-file-read");
...
static int s3_multipart_file_state_validate(const s3_multipart_impl *upload, uint64_t file_size,
  ...
  for (index = 0u; index < upload->part_capacity; ++index) {
    ...
    if (upload->parts[index].etag == NULL) continue;
    if (index >= part_count || offset >= file_size) return TURBO_EPROTO;
    expected_size =
        file_size - offset < (uint64_t)part_size ? (size_t)(file_size - offset) : part_size;
    if (upload->parts[index].size != expected_size) return TURBO_EPROTO;
  }
  return TURBO_OK;
}
...
static int s3_multipart_file_failure(s3_client *client, s3_multipart *upload,
  ...
  if (preserve_on_failure) {
    ...
    if (abort_status == TURBO_OK) {
      (void)turbo_fs_unlink(checkpoint_path);
    } else {
      (void)s3_multipart_detach(upload);
...
int s3_put_object_multipart_file(s3_client *client, const char *bucket, const char *key,
  ...
  *out_error = (s3_error){0};
  if (client_impl == NULL || bucket == NULL || key == NULL || path == NULL || path[0] == '\0' ||
    ...
    return s3_multipart_error(out_error, TURBO_EINVAL, "multipart-file-options");
  if (client_impl->operation_active)
    return s3_multipart_error(out_error, TURBO_EBUSY, "multipart-file-options");
  ...
  if (part_size < S3_MULTIPART_MIN_PART_BYTES ||
    ...
    return s3_multipart_error(out_error, TURBO_EINVAL, "multipart-file-part-size");
  status = turbo_fs_stat(path, &file_stat);
  if (status != TURBO_OK || !file_stat.is_file || file_stat.size == 0u ||
      file_stat.size > SIZE_MAX || file_stat.size > INT64_MAX)
    return s3_multipart_error(out_error, status != TURBO_OK ? TURBO_EIO : TURBO_EINVAL,
                              "multipart-file-stat");
  ...
  if (part_count == 0u || part_count > client_impl->base.config.max_multipart_parts)
    return s3_multipart_error(out_error, TURBO_ENOBUFS, "multipart-file-parts");
  file = turbo_fs_open(path, TURBO_FS_O_RDONLY, 0);
  if (file == TURBO_INVALID_FILE)
    return s3_multipart_error(out_error, TURBO_EIO, "multipart-file-open");
  buffer = (unsigned char *)malloc(part_size);
  if (buffer == NULL) {
    (void)turbo_fs_close(file);
    return s3_multipart_error(out_error, TURBO_ENOMEM, "multipart-file-buffer");
  }
  if (options->resume_existing) {
    ...
    if (status == TURBO_OK) checkpoint_available = 1;
    if (status != TURBO_OK) s3_multipart_error(out_error, status, "multipart-checkpoint-load");
  } else {
    const int checkpoint_status = turbo_fs_stat(options->checkpoint_path, &checkpoint_stat);
    if (checkpoint_status == TURBO_OK) {
      status = s3_multipart_error(out_error, TURBO_EALREADY, "multipart-checkpoint-exists");
    } else if (checkpoint_status != -ENOENT) {
      status = s3_multipart_error(out_error, TURBO_EIO, "multipart-checkpoint-stat");
    } else {
      ...
      if (status == TURBO_OK) {
        ...
        if (status == TURBO_OK) checkpoint_available = 1;
        if (status != TURBO_OK) s3_multipart_error(out_error, status, "multipart-checkpoint-store");
      }
    }
  }
  upload_impl = (s3_multipart_impl *)upload.impl;
  ...
  status = turbo_fs_stat(path, &final_stat);
  if (status != TURBO_OK || !final_stat.is_file || final_stat.size != file_stat.size ||
      final_stat.mtime != file_stat.mtime || final_stat.ctime != file_stat.ctime) {
    status = s3_multipart_error(out_error, TURBO_EBUSY, "multipart-file-changed");
    goto failed;
  }
  close_status = turbo_fs_close(file);
  file = TURBO_INVALID_FILE;
  if (close_status != TURBO_OK) {
    status = s3_multipart_error(out_error, TURBO_EIO, "multipart-file-close");
    goto failed;
  }
  status = s3_multipart_complete(client, &upload, out_response, out_error);
  if (status != TURBO_OK) goto failed;
  if (turbo_fs_unlink(options->checkpoint_path) != TURBO_OK)
    status = s3_multipart_error(out_error, TURBO_EIO, "multipart-checkpoint-remove");
...
failed:
  if (file != TURBO_INVALID_FILE) (void)turbo_fs_close(file);
  OPENSSL_cleanse(buffer, part_size);
  free(buffer);
...
int s3_multipart_detach(s3_multipart *upload) {
  s3_multipart_impl *impl = upload != NULL ? (s3_multipart_impl *)upload->impl : NULL;
  if (impl == NULL || impl->state != S3_MULTIPART_ACTIVE) return TURBO_EINVAL;
...
int s3_multipart_destroy(s3_multipart *upload) {
  ...
  if (upload == NULL) return TURBO_EINVAL;
  impl = (s3_multipart_impl *)upload->impl;
  if (impl == NULL) return TURBO_OK;
  if (impl->state == S3_MULTIPART_ACTIVE) return TURBO_EBUSY;
