...
int s3_put_object_with_options(s3_client *client, const char *bucket, const char *key,
  ...
  if (data == NULL && size != 0u) {
    if (out_error != NULL)
      *out_error = (s3_error){.status = TURBO_EINVAL, .stage = "s3-put-object"};
    return TURBO_EINVAL;
  }
  ...
  if (status == TURBO_OK && options->content_type != NULL)
    headers[header_count++] = (chttp_header){"Content-Type", options->content_type};
  for (index = 0u; index < sse_headers.count && status == TURBO_OK; ++index)
  ...
  if (status != TURBO_OK && out_error != NULL && out_error->status == TURBO_OK)
...
static int s3_read_object_with_options(s3_client *client, s3_method method, const char *bucket,
  ...
  if (impl == NULL || (options != NULL && options->size != sizeof(*options))) {
    if (out_error != NULL)
      *out_error = (s3_error){.status = TURBO_EINVAL, .stage = "s3-read-object"};
    return TURBO_EINVAL;
  }
...
int s3_copy_object(s3_client *client, const char *source_bucket, const char *source_key,
  ...
  if (impl == NULL || out_response == NULL || out_error == NULL || source_bucket == NULL ||
    ...
    if (out_error != NULL)
      *out_error = (s3_error){.status = TURBO_EINVAL, .stage = "s3-copy-object"};
    return TURBO_EINVAL;
  }
  ...
  if (status == TURBO_OK &&
      (s3_checked_add(required, tstr_len(encoded_bucket), &required) != TURBO_OK ||
       s3_checked_add(required, tstr_len(encoded_key), &required) != TURBO_OK ||
  ...
  if (status == TURBO_OK) status = s3_text_builder_append_cstr(&builder, encoded_bucket);
  if (status == TURBO_OK) status = s3_text_builder_append(&builder, "/", 1u);
...
static int s3_object_info_parse(turbo_xml_node node, s3_object_info *out) {
  const turbo_xml_node key = s3_xml_child(node, "Key");
  const turbo_xml_node modified = s3_xml_child(node, "LastModified");
  const turbo_xml_node etag = s3_xml_child(node, "ETag");
  const turbo_xml_node size = s3_xml_child(node, "Size");
  const turbo_xml_node storage = s3_xml_child(node, "StorageClass");
  char *size_text;
  int status;
  if (key.impl == NULL || size.impl == NULL || out == NULL) return TURBO_EPROTO;
  size_text = NULL;
  status = s3_xml_text_dup(key, &out->key);
  if (status == TURBO_OK && modified.impl != NULL)
    status = s3_xml_text_dup(modified, &out->last_modified);
  if (status == TURBO_OK && etag.impl != NULL) status = s3_xml_text_dup(etag, &out->etag);
  if (status == TURBO_OK && storage.impl != NULL)
    status = s3_xml_text_dup(storage, &out->storage_class);
  if (status == TURBO_OK) status = s3_xml_text_dup(size, &size_text);
  if (status != TURBO_OK) {
    free(size_text);
    return status;
  }
...
static int s3_object_list_parse(const s3_client_impl *client, const s3_response *response,
  ...
  turbo_xml_node truncated;
  turbo_xml_node next_token;
  size_t count = 0u;
  size_t index;
  ...
  if (status == TURBO_OK && truncated.impl != NULL) {
    char *value = NULL;
    status = s3_xml_text_dup(truncated, &value);
    if (status == TURBO_OK && strcmp(value, "true") == 0) out->is_truncated = 1;
    else if (status == TURBO_OK && strcmp(value, "false") != 0) status = TURBO_EPROTO;
    free(value);
  }
  next_token = s3_xml_child(root, "NextContinuationToken");
  if (status == TURBO_OK && next_token.impl != NULL)
    status = s3_xml_text_dup(next_token, &out->next_continuation_token);

done:
  turbo_xml_document_destroy(&document);
  if (status != TURBO_OK) s3_object_list_destroy(out);
  return status;
}
...
int s3_list_objects(s3_client *client, const char *bucket, const s3_list_objects_options *options,
  ...
  *out_list = (s3_object_list){0};
  ...
  if (options != NULL && options->max_keys != 0u) {
    ...
    if (written <= 0 || (size_t)written >= sizeof(max_keys)) return TURBO_ERANGE;
...
int s3_presign_url(s3_client *client, s3_method method, const char *bucket, const char *key,
  ...
  *out_error = (s3_error){0};
  if (impl == NULL || out_url == NULL || *out_url != NULL) return TURBO_EINVAL;
  ...
  if (status != TURBO_OK) *out_error = (s3_error){.status = status, .stage = "s3-presign"};
  return status;
}
