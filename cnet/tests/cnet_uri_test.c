...
spec("CNet strict transport URI") {
  ...
  it("parses a bounded pipe name without inventing an authority") {
    ...
    check_equal(cnet_uri_parse("pipe://[service]", &uri), TURBO_OK);
    check_equal(strcmp(uri.path, "[service]"), 0);

    check_equal(cnet_uri_parse("pipe://name:segment", &uri), TURBO_OK);
  ...
  it("rejects ambiguous unsupported and overflowing forms") {
    ...
    check_equal(cnet_uri_parse("tcp://host:999999999999999999999", &uri), TURBO_ERANGE);
    check_equal(cnet_uri_parse("tcp://host:80/path", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://host:80?query", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://host:80#fragment", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://user@host:80", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://@host:80", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://host:80?", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://host:80#", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://host:", &uri), TURBO_EINVAL);
    check_equal(cnet_uri_parse("tcp://bad%zz:80", &uri), TURBO_EINVAL);
    ...
    check_equal(cnet_uri_parse(oversized_network, &uri), TURBO_ERANGE);
