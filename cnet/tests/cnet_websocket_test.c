...
spec("CNet WebSocket session") {
  it("requires explicit valid limits and callbacks") {
    ...
    check_equal(cnet_websocket_init(NULL, &config), SALTS_EINVAL);
    check_equal(cnet_websocket_init(&websocket, NULL), SALTS_EINVAL);
    config.size = 0u;
  ...
  it("delivers masked server input split across feed calls") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, 1u), SALTS_OK);
    check_equal(probe.event_count, 0u);
    check_equal(cnet_websocket_feed(&websocket, wire + 1u, wire_size - 1u), SALTS_OK);
  ...
  it("processes coalesced binary and pong frames") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, first + second), SALTS_OK);
  ...
  it("reassembles fragments while answering an interleaved ping") {
    ...
    total += websocket_test_frame(wire + total, sizeof(wire) - total, WS_OPCODE_CONTINUATION, 1, 1,
                                  "lo", 2u);
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, total), SALTS_OK);
  ...
  it("fails role masking violations with protocol close") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, wire_size), SALTS_EPROTO);
  ...
  it("fails invalid reassembled UTF-8 with close code 1007") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, wire_size), SALTS_ECHARSET);
    check_equal(probe.event_count, 0u);
  ...
  it("rejects a new data message during fragmented input") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, total), SALTS_EPROTO);
    check_equal(websocket_test_close_code(probe.output[0], probe.output_size[0]), 1002u);
    check_equal(cnet_websocket_destroy(&websocket), SALTS_OK);
  }
  ...
  it("bounds a reassembled fragmented message") {
    ...
    total += websocket_test_frame(wire + total, sizeof(wire) - total, WS_OPCODE_CONTINUATION, 1, 1,
                                  "56", 2u);
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, total), SALTS_EMSGSIZE);
  ...
  it("rejects an announced frame beyond the configured limit without its payload") {
    ...
    check_equal(ws_frame_build_header(header, sizeof(header), WS_OPCODE_BINARY,
                ...
                WS_PARSE_OK);
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, header, header_size), SALTS_EMSGSIZE);
    check_equal(websocket_test_close_code(probe.output[0], probe.output_size[0]), 1009u);
    check_equal(cnet_websocket_destroy(&websocket), SALTS_OK);
  }
  ...
  it("masks client output and retains one frame across transport backpressure") {
    ...
    check_equal(cnet_websocket_flush(&websocket), SALTS_OK);
  ...
  it("does not consume caller input while output is backpressured") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, ping, ping_size), SALTS_OK);
    check_true(cnet_websocket_has_pending_output(&websocket));
    check_equal(cnet_websocket_feed(&websocket, message, message_size), SALTS_EBUSY);
    check_equal(probe.event_count, 1u);
    check_equal(cnet_websocket_flush(&websocket), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, message, message_size), SALTS_OK);
    check_equal(probe.event_count, 2u);
  ...
  it("keeps feed input atomic when the configured buffer is full") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, 1u), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, rejected, sizeof(rejected)), SALTS_ENOSPC);
    check_equal(cnet_websocket_feed(&websocket, wire + 1u, wire_size - 1u), SALTS_OK);
    check_equal(probe.event_count, 1u);
  ...
  it("validates outbound text across fragment boundaries") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_send_text(&websocket, invalid, sizeof(invalid)), SALTS_ECHARSET);
  ...
  it("echoes a valid peer close and completes the closing handshake") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, wire_size), SALTS_OK);
    check_equal(probe.event_count, 1u);
  ...
  it("commits peer closing state before delivering its close event") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, wire_size), SALTS_OK);
    check_equal(probe.nested_send_status, SALTS_ESHUTDOWN);
  ...
  it("allows callback sends but rejects recursive lifecycle mutation") {
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, wire_size), SALTS_OK);
    check_equal(probe.nested_send_status, SALTS_OK);
    check_equal(probe.nested_destroy_status, SALTS_EBUSY);
    check_equal(probe.output_count, 1u);
    check_equal(ws_frame_parse(probe.output[0], probe.output_size[0], &frame), WS_PARSE_OK);
  ...
  it("propagates a terminal nested callback write error from feed") {
    cnet_websocket websocket = {0};
    websocket_probe probe = {.write_status = SALTS_EIO, .nested_event_ops = true};
    cnet_websocket_config config = websocket_test_config(&probe, CNET_WEBSOCKET_SERVER);
    uint8_t wire[32];
    size_t wire_size = websocket_test_frame(wire, sizeof(wire), WS_OPCODE_TEXT, 1, 1, "event", 5u);
    cnet_websocket_state state = CNET_WEBSOCKET_OPEN;
    ...
    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_feed(&websocket, wire, wire_size), SALTS_EIO);
    check_equal(probe.nested_send_status, SALTS_EIO);
    check_equal(probe.nested_destroy_status, SALTS_EBUSY);
    check_equal(cnet_websocket_state_get(&websocket, &state), SALTS_OK);
    check_equal(state, CNET_WEBSOCKET_FAILED);
    check_equal(cnet_websocket_last_error(&websocket, &last_error), SALTS_OK);
    check_equal(last_error, SALTS_EIO);
    check_equal(cnet_websocket_destroy(&websocket), SALTS_OK);
  }

  it("records terminal transport write errors") {
    cnet_websocket websocket = {0};
    websocket_probe probe = {.write_status = SALTS_EIO};
    cnet_websocket_config config = websocket_test_config(&probe, CNET_WEBSOCKET_SERVER);
    cnet_websocket_state state = CNET_WEBSOCKET_OPEN;
    int last_error = SALTS_OK;

    check_equal(cnet_websocket_init(&websocket, &config), SALTS_OK);
    check_equal(cnet_websocket_send_ping(&websocket, "x", 1u), SALTS_EIO);
    check_equal(cnet_websocket_state_get(&websocket, &state), SALTS_OK);
    check_equal(state, CNET_WEBSOCKET_FAILED);
    check_equal(cnet_websocket_last_error(&websocket, &last_error), SALTS_OK);
    check_equal(last_error, SALTS_EIO);
