...
spec("CNet benchmark paired statistics") {
  it("reports the median and MAD without pooling independent runs") {
    ...
    check_equal(cnet_benchmark_summarize(values, 5u, &summary), TURBO_OK);
  ...
  it("computes deltas from matched baseline and candidate runs") {
    ...
    check_equal(cnet_benchmark_summarize_paired_delta(baseline, candidate, 5u, &summary), TURBO_OK);
    check_equal(summary.median, 20.0);
    check_equal(summary.mad, 10.0);
  }
  ...
  it("rejects invalid or non-finite samples") {
    ...
    check_equal(cnet_benchmark_summarize(NULL, 1u, &summary), TURBO_EINVAL);
    check_equal(cnet_benchmark_summarize(invalid, 2u, &summary), TURBO_ERANGE);
  }
}
