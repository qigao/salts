...
enum {
  ...
  EPOLL_TEST_BLOCKED_WINDOW_NS = 20000000ULL,
...
spec("Platform epoll readiness") {
  ...
  it("filters stale events when a descriptor number is reused") {
    ...
    check_equal(turbo_readiness_arm(&new_registration, TURBO_READINESS_EVENT_READ, record_callback,
                                    &new_probe),
                TURBO_OK);
    check_equal(write_byte(new_pipe[1], 5), TURBO_OK);
  ...
  it("shutdown joins the reactor thread after an inflight callback") {
    ...
    while (!shutdown_args.completed && wait_status == TURBO_OK)
      wait_status = turbo_cond_timedwait(&shutdown_args.changed, &shutdown_args.mutex,
                                         EPOLL_TEST_BLOCKED_WINDOW_NS);
