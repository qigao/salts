...
- **io_uring**：prepare 将有界描述符放入原 request 槽位；只有每个 read/write lane 的 head
  ...
  blocking observe 在需要时把 wake eventfd 的 `IORING_OP_POLL_ADD` 与用户 SQE 放入同一批，
  其 CQE 只作为控制唤醒，不暴露用户 completion；wake 返回后不做 control-only rearm，
  下一次 blocking observe 再随批提交。若内部 control poll 因 owner task handoff 或内核能力被取消/拒绝，
  只把等待策略降级为
  `poll(ring_fd, wake_fd)`，不把控制 CQE 当成用户 I/O 失败，也不切换 backend。更老 kernel
  缺少 EXT_ARG 时有限 timeout 使用同一兼容等待路径。
