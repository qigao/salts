...
if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
  throw "UDP DATAGRAM style benchmark CSV not found: $Path"
}

$rows = @(Import-Csv -LiteralPath $Path)
$styles = @("direct", "coroutine", "sharded_same_owner", "sharded_cross_owner")
$payloads = @(1024, 4096, 8192)
$expectedRows = $styles.Count * $payloads.Count
if ($rows.Count -ne $expectedRows) {
  throw "expected $expectedRows UDP DATAGRAM style rows, got $($rows.Count)"
}
...
foreach ($name in $required) {
  if (-not ($rows[0].PSObject.Properties.Name -contains $name)) {
    throw "missing UDP DATAGRAM style benchmark column: $name"
  }
}
...
foreach ($row in $rows) {
  ...
  if ($styles -notcontains $row.style) {
    throw "unexpected UDP DATAGRAM style: $($row.style)"
  }

  $payload = Parse-U64 $row.payload_bytes "payload_bytes"
  if ($payloads -notcontains [int]$payload) {
    throw "unexpected UDP DATAGRAM payload: $payload"
  }
  $key = "$($row.style)|$payload"
  if ($seen.ContainsKey($key)) {
    throw "duplicate UDP DATAGRAM style cell: $key"
  }
  $seen[$key] = $true
  ...
  if ($rejected -ne 0) {
    throw "measured UDP DATAGRAM style cell rejected accepted work: $key"
  }

  switch ($row.style) {
    "direct" {
      if ($hops -ne 0.0 -or $direct -ne 0.0 -or $queued -ne 0 -or $peak -ne 0) {
        throw "Direct UDP DATAGRAM cell must not carry Sharded routing counters: $key"
      }
    }
    "coroutine" {
      if ($hops -ne 0.0 -or $direct -ne 0.0 -or $queued -ne 0 -or $peak -ne 0) {
        throw "Coroutine UDP DATAGRAM cell must not carry Sharded routing counters: $key"
      }
    }
    "sharded_same_owner" {
      if ($hops -ne 0.0 -or $queued -ne 0) {
        throw "same-owner Sharded UDP DATAGRAM must remain zero-hop: $key"
      }
      if ($direct -lt 2.0) {
        throw "same-owner Sharded UDP DATAGRAM must exercise at least read+write direct tasks/transfer: $key"
      }
      if ($peak -eq 0) {
    ...
    "sharded_cross_owner" {
      if ($direct -ne 0.0) {
        throw "cross-owner Sharded UDP DATAGRAM must not use same-owner direct routing: $key"
      }
      if ($hops -lt 3.0) {
        throw "cross-owner Sharded UDP DATAGRAM must route read, write, and owner observe: $key"
      }
...
foreach ($style in $styles) {
  foreach ($payload in $payloads) {
    ...
    if (-not $seen.ContainsKey($key)) {
      throw "missing UDP DATAGRAM style cell: $key"
    }
  }
}

Write-Host "NativeIO UDP DATAGRAM execution-style benchmark structure verified: backend=$($rows[0].backend)"
