...
/**
 * Text serializers require the matching codec and always apply the primary
 * schema `[name(...)]` mapping. `[alias(...)]` remains input-only, while
 * DataBindObject continues to store canonical schema field names internally.
 */
 ...
 * data_bind_parse(). A successfully constructed record is immutable and does
 * not borrow its schema layout from the codec. All field,
