...
#define DATA_BIND_VERSION_MINOR 12
...
/**
 * @brief Create a codec from schema text and a previously generated textual MIR module.
 * @param schema_text Exact schema text used to generate the MIR module
 * @param schema_len Length of schema text
 * @param mir_text Textual MIR bytes generated by data_bind_generate_mir()
 * @param mir_len Length of textual MIR bytes
 * @param out_codec Output parameter for the created codec
 * @param error Output parameter for error information
 * @return Status code. On success, *out_codec must be freed with data_bind_free().
 *
 * The generated module carries a parser ABI version and a fingerprint of the
 * schema bytes. Loading fails when either value does not match. This path skips
 * parser generation but still JIT-links the loaded MIR module. Schema and MIR
 * inputs must both come from trusted sources.
 */
DATA_BIND_API DataBindStatus data_bind_create_from_mir(
    const char *schema_text, size_t schema_len, const char *mir_text, size_t mir_len,
    DataBind **out_codec, DataBindError *error);

/**
 * @brief Create a codec from schema text and a previously generated binary MIR module.
 * @param schema_text Exact schema text used to generate the BMIR module
 * @param schema_len Length of schema text
 * @param bmir_data Binary MIR bytes generated by data_bind_generate_mir()
 * @param bmir_len Length of binary MIR bytes
 * @param out_codec Output parameter for the created codec
 * @param error Output parameter for error information
 * @return Status code. On success, *out_codec must be freed with data_bind_free().
 *
 * The generated module carries a parser ABI version and a fingerprint of the
 * schema bytes. Loading fails when either value does not match. This path skips
 * parser generation but still JIT-links the loaded module. Schema and BMIR
 * inputs must both come from trusted sources.
 */
DATA_BIND_API DataBindStatus data_bind_create_from_bmir(
    const char *schema_text, size_t schema_len, const void *bmir_data, size_t bmir_len,
    DataBind **out_codec, DataBindError *error);
