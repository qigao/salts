...
**Tech Stack:** C11、POSIX poll/pipe/fcntl、TurboUtils Platform threads/readiness、CFlow I/O Actor、TinyTest、CMake Presets、GitHub Actions。
...
- poll 只在 POSIX 编译，显式选择不支持的 kind 返回 `TURBO_ENOTSUP`，绝不 fallback。
...
**Interfaces:**
- Produces: `turbo_readiness_backend_kind`, `turbo_readiness_backend_supported()`, `turbo_readiness_reactor_init_kind()`, `CFLOW_IO_NATIVE_POLL`。
- Preserves: `turbo_readiness_reactor_init()` 的平台默认选择。

- [ ] **Step 1: 写 selector 与 unsupported 的失败测试**

在 Windows 测试中断言 poll compile-time unsupported 且显式 init 返回
`TURBO_ENOTSUP`、清空非零 reactor；在 C/C++ header test 中引用两个 additive enum。
...
`turbo_readiness_reactor_init_kind()` 对已编译的 epoll/kqueue 调现有 factory，其他 kind
返回 `TURBO_ENOTSUP`；default initializer 调用同一 selector。CFlow 追加 enum、supported
...
**Interfaces:**
- Consumes: `turbo_readiness_backend_ops` 与 `turbo_readiness_reactor_init_backend()`。
- Produces: internal `turbo_readiness_poll_init()`；固定 records/snapshot/control-pipe worker。

- [ ] **Step 1: 写 real pipe/socket poll contract tests**

复用 `readiness_backend_contract.c`，factory 必须通过
`turbo_readiness_reactor_init_kind(..., TURBO_READINESS_BACKEND_POLL)` 初始化；额外覆盖
write readiness、POLLHUP/POLLNVAL 映射、descriptor close/reuse 和 blocked callback shutdown。

- [ ] **Step 2: 在 POSIX CI/host 构建中记录 RED**

预期：explicit poll init 为 `TURBO_ENOTSUP` 或 test target 缺 source。
...
`turbo_readiness_reactor_init_kind()`；CMake 在全部 POSIX 编译一次 shared adapter，并按平台
