...
cmake --build --preset build-debug-linux --target cmeta_core_test
```
...
```bash
cmake --build --preset build-debug-linux --target cmeta_core_test
ctest --preset test-dev-linux -R '^cmeta_core_test$'
```
...
```bash
cmake --build --preset build-debug-linux --target cmeta_collector_test
```
...
```bash
cmake --build --preset build-debug-linux --target turbostl_header_typed_test
```
...
cmake --build --preset build-debug-linux --target \
  ...
  turbostl_map_test \
  turbostl_hash_test
ctest --preset test-dev-linux -R '^turbostl_'
git diff --check
```
...
```bash
git add turbostl/tests/turbostl_header_typed_cpp_test.cpp turbostl/tests/install_consumer
