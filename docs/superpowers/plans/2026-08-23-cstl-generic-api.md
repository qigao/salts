# Container Generic API Implementation Plan

> **For Codex:** Execute this plan task-by-task with test-driven development and fresh verification before completion.

**Goal:** Restore Container's finite CMeta Generic API while retaining PR #53's self-describing raw-handle initializers and make typed Stream terminals explicitly name their output container.

**Architecture:** CMeta continues to route `typed(kind, ...)`; a Container-owned schema generates concrete wrappers over compiled raw handles. PR #53 declaration/expression forms remain erased-handle initializers. Generated `Type_method` functions and distinctly named typed Stream terminals avoid shadowing raw List/Map calls or three-argument Stream terminals.

**Tech Stack:** C11, CMeta Schema/macros, Container, CFlow, CMake Presets, TinyTest.

**Spec:** `docs/superpowers/specs/2026-08-23-container-generic-api-design.md`
...
- [x] Build the focused targets and confirm they fail because Container kinds and typed terminals are unavailable.

### Task 2: Restore Container-owned finite facade generation

**Files:**
- Create: `cstl/include/cstl/detail/typed_initializers.h`
- Create: `cstl/include/cstl/detail/typed_facade.h`
- Modify: `cstl/include/cstl/typed.h`

- [x] Move the Container-specific kind schema and generator adapters behind a Container detail header.
...
- [x] Build and run `cstl_stream_test`.

### Task 4: Migrate typed-facing coverage and documentation

**Files:**
- Modify: `cstl/tests/cstl_header_test.c`
- Modify: `cstl/tests/cstl_header_typed_test.c`
- Modify: `cstl/tests/cstl_typed_test.c`
- Modify: `cstl/tests/cstl_entry_test.c`
- Modify: `cstl/tests/cstl_generic_identity_test.c`
- Modify: `cstl/tests/cstl_semantic_projection_test.c`
- Modify: `cstl/tests/cstl_list_test.c`
- Modify: `cstl/tests/cstl_map_test.c`
- Modify: `cstl/tests/cstl_tree_test.c`
...
- [x] Run all Container tests plus adjacent CMeta/CFlow tests.
...
- [x] Push `feat/container-generic-api` and open a PR against `master` with the design, compatibility impact, and exact verification evidence.
