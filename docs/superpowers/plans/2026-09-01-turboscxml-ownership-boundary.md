...
**Goal:** Make `qigao/turbo-scxml` the sole SCXML implementation while TurboUtils retains only the format-neutral CFlow StateChart host protocol.

**Architecture:** Delete the duplicated interpreter as one atomic package-boundary change. Remove every CMake and installed-consumer route to the old target, replace the embedded CFlow README section with a migration pointer, then prove that standalone TurboSCXML consumes the resulting TurboUtils SDK.
...
- `TurboSCXML -> TurboUtils::CFlow` is the only permitted dependency direction.
...
**Interfaces:**
- Consumes: existing `TurboUtils::CFlow` package target.
- Produces: a TurboUtils package with no SCXML option, header, library, or exported target.
...
```powershell
rg.exe -n "CFLOW_ENABLE_SCXML|CFlowScxml|CONSUME_CFLOW_SCXML|EXPECT_CFLOW_SCXML|TURBOUTILS_EXPECT_CFLOW_SCXML" CMakeLists.txt CMakeOptions.cmake cmake tests
```
...
`TurboUtils::CFlowScxml` package instructions.
...
```powershell
rg.exe -n "TurboUtils::CFlowScxml|CFLOW_ENABLE_SCXML|<cflow/scxml.h>" cflow
```
...
**Files:**
- Test: TurboUtils configured build and installed consumer.
- Test: standalone TurboSCXML configured against the generated TurboUtils package.

**Interfaces:**
- Consumes: installed `TurboUtils::CFlow`, `CMeta`, `XmlParser`, `Core`, and `QueryVM`.
- Produces: passing TurboUtils and TurboSCXML build/test/package evidence.

- [x] **Step 1: Configure TurboUtils from a clean Release tree**
...
- [x] **Step 2: Build and test TurboUtils**
...
Use the standalone repository's documented preset with `TURBOUTILS_ROOT`
...
`TurboUtils::CFlowScxml` is intentionally removed in favor of TurboSCXML.
