# Container Expression Initializers Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add allocation-free expression initializers for every public Container container while preserving the existing declaration DSL.
...
**Tech Stack:** Strict C11, CMeta finite type descriptors, Container self-describing handles, TinyTest, CMake Presets, MSVC C mode.
...
**Files:**
- Modify: `cstl/tests/cstl_header_typed_test.c`
...
Add two cases to `cstl_header_typed_test.c`: one assigns every unary `*Of(T)` value and verifies `cmeta_container_type_application_valid`, and one does the same for all binary `*Of(K,V)` values. Use assignment for `VecOf` and `MapOf` so an initializer-list-only macro cannot satisfy the contract.
...
```bat
cmake --build --preset build-release-windows --target cstl_header_typed_test
```
...
In `cstl/typed.h`, define kind-specific brace initializer facts and guarded public `*Of` compound-literal macros. Refactor existing declaration macros to consume the same kind-specific brace initializer without changing their names or resulting handle layouts.
...
```bat
cmake --build --preset build-release-windows --target cstl_header_typed_test
ctest --preset test-release-windows -R "^cstl_header_typed_test$" --output-on-failure
```
...
```bat
cmake --build --preset build-release-windows --target cstl_header_typed_test cstl_header_typed_cpp_test cstl_construction_binding_test cstl_typed_test
ctest --preset test-release-windows -R "^(cstl_header_typed_test|cstl_header_typed_cpp_test|cstl_construction_binding_test|cstl_typed_test)$" --output-on-failure
```
...
Run `git diff --check`, inspect `git diff`, then commit the plan, test, header, and documentation with message `feat(container): add expression initializers`.
