...
- [x] Define the C schema with explicit-width version/opcode/path fields, retained `cmeta_callable` values, semantic type descriptors, property masks, Graph version/fingerprint, and encounter-order policy. Use owned contiguous rows and checked `count * sizeof(row)` allocation; state that the instance is runtime-only.
- [x] Define Lean `CertificateVersion`, `CertifiedOpcode`, `CertifiedPath`, `CertificateRow`, and `PlanCertificate` with numerals matching C `_Static_assert` values.
- [x] Add C compile-time assertions and Lean examples for every stable numeral.
- [x] Build CFlow and run `lake test`; expected GREEN only when both representations are complete.
- [x] Commit as `feat(cflow): define plan refinement certificate`.
...
- [x] Write a passing linear Filter/Map/Reduce case and negative copies tampering one field at a time: version, Graph fingerprint, opcode, type, callable identity, property, path, order policy, and row count.
- [x] Run the new test and confirm RED because the checker is absent.
- [x] Hash canonical normalized Graph topology/opcode/property numerals only; do not hash pointer addresses or padding. Compare callable and type semantics with `cmeta_callable_same()` and `cmeta_type_equal()`. Recompute and compare every row against Plan instructions and Graph traversal before setting a local `matched` flag.
- [x] Clear `*error` on success, return the first stable static diagnostic on failure, and never modify Graph/Plan/certificate during checking.
- [x] Add unsupported Relation and stale-Graph tests; verify Kernel remains independently executable.
- [x] Run certificate, pipeline, optimizer, Direct, and calculus conformance tests.
- [x] Commit as `feat(cflow): check plan refinement certificates`.
...
- [x] Define certificate validity as exact ordered agreement between Graph semantic rows, Plan rows, type chain, callable identity, and required properties.
- [x] Prove by row induction that a valid sequential certificate has the same observation as its normalized Graph denotation.
- [x] Define contiguous nonempty chunks and prove ordered left-combination equals sequential left reduction under associativity. Do not assume commutativity or permit chunk permutation.
- [x] Lift the theorem to the Parallel Reduce certificate path and reuse `ExecutionRefines.r11` for observation preservation.
- [x] Add negative examples showing missing associativity, reversed chunk order, stale graph identity, and unsupported WAIT cannot construct validity.
- [x] Run `lake test`; scan production formal modules with `rg.exe -n "sorry|admit|axiom"` and require no new matches.
- [x] Commit as `formal(cflow): prove plan certificate refinement`.
...
- [x] Add `formal/cmeta_cflow_calculus/**` to workflow path filters.
- [x] Install Lean through the repository-pinned `lean-toolchain`, run `lake test`, then run `cflow_certificate_test` and `cflow_calculus_conformance_test` in the same job.
- [x] Record the exact trust boundary: Lean theorem, C checker tests, compiler/runtime outside proof.
- [x] Run local `lake test`, focused CTests, full Windows Release CTest, installed-package verification, and `git diff --check`.
- [x] Commit as `ci(cflow): verify Lean C refinement certificates`.
