...
> 定位更新（2026-08-25）：Stream 的目标是为 Container 容器提供便利、现代、
...
- `utils/`（`Salts::Core`）编译并安装一组基础容器；
- 旧 `salts/` 目录包含更完整的容器集合及 typed 宏；
- 旧 `stream/` 提供一套独立的 Java 风格有限流和 live/SPSC 运行时，而 `cflow/` 已经拥有 typed graph、runtime、source、scheduler 与 fluent stream facade。

当前工作区已经开始删除旧 `salts/`、`stream/` 并建立 `cstl/`，但尚未完成 target、安装、测试、调用方和错误/所有权协议的迁移。本设计以这些未提交改动为迁移基线，不恢复第二套容器或 Stream runtime。

## 目标

1. 将 Core 中的标准容器和旧 `salts/` 容器统一迁入 `cstl/`，使其成为唯一标准容器库。
2. 以 CMeta `typed(...)` 和显式 traits 生成 typed facade、Range 与 collector 能力。
3. 由 Container 提供 CFlow 的容器实现适配，使有限容器获得现代化、lazy、类型安全且资源有界的 pipeline 与 collection API。
4. 保持 CFlow 不依赖具体 Container；STLCFlow 在构造 stream 时显式注入状态后端和 collector。
5. 对所有增长状态设置硬上限，明确所有权、失效点、错误、清理与重复执行语义。
6. 删除旧公开 include、typed 宏和 `Salts::Stream` target，不保留兼容层。
...
- 不让 CFlow include、链接或运行时发现 Salts 容器。
...
标准容器、CFlow 抽象和二者适配各有独立 target。CMeta 承载有限的类型与能力描述，STLCFlow 通过显式 Adapter/Strategy 将 Container 接入 CFlow。

优点是无依赖环、无全局注册、无算法复制，Core 也能只消费标准容器本体。代价是新增一个明确的 `Salts::CSTLCFlow` target，并需要一次完整的 breaking migration。
...
Salts::CSTL      -> Salts::CMeta
Salts::CSTLCFlow -> Salts::CSTL + Salts::CFlow
Salts::Core           -> Salts::CSTL（PRIVATE）
```

职责如下：

- `Salts::CMeta`：类型描述、type traits、Range、container descriptor 与 collector 协议。
- `Salts::CFlow`：graph、operator、terminal、runtime、status 和抽象的有界状态后端协议。
- `Salts::CSTL`：标准容器算法、raw API、typed facade、Range/collector adapter。
- `Salts::CSTLCFlow`：Container 对 CFlow stream、state backend 和 typed collector 的实现。
- `Salts::Core`：错误、字符串、文件、平台、日志、automata 等共享能力；不再拥有或安装标准容器。

Container 不依赖 Core。它提供自己的 export header 与 `salts_stl_status`，避免 Core 私有消费 Container 时产生依赖环。Core 在自身公开边界把 Container 错误转换为现有 `SALTS_E*`。
...
`<cstl.h>` 是标准容器聚合头；`<cstl/cflow.h>` 是可选 CFlow 集成入口。旧 flat include 路径不安装。
...
```text
salts_buffer / memory_pool / object_pool
...
Core 内的 `ac_automaton`、`levenshtein_automaton` 以及 `salts_serial` 等调用方改用新 include 和 target，不复制或私藏 Vec 实现。
...
`List` 不包含 `salts_deque_t`，不公开 `reserve`、`capacity` 或随机访问 `at(index)`。它公开双向 iterator、`insert_before/after` 和 `erase`；插入不使既有节点地址失效，删除只使被删除节点失效，`clear/destroy` 使全部节点失效。

`Map` 不 typedef、不组合 `salts_hash_map_t`，也不接收 hash/equal。它从 key type traits 取得 comparator，并公开 `lower_bound`、`upper_bound`、有序 iterator 和唯一 key 的替换语义。`Set` 复用同一内部红黑树引擎但没有 value。`MultiMap` 的内部比较键为 `(user_key, insertion_sequence)`，从而让相同 key 连续且保持插入顺序；sequence 溢出必须在修改前失败。
...
- raw 类型名如 `salts_vec_t`、`salts_hash_map_t` 保留。
- raw API 改用 Container 自身 status 和显式类型/字节语义，不再从 Core 取得错误码。
- typed 声明的唯一公开入口是 `typed(Kind, ...)`。
- `SALTS_VEC_DEFINE`、`SALTS_HASH_MAP_DEFINE` 等宏降为 Container 内部生成机制，不再安装或文档化。
- include 统一迁移至 `<cstl/...>`。
- 删除 `Salts::Stream`；普通用户链接 `Salts::CSTL`，容器流用户链接 `Salts::CSTLCFlow`。
...
所有容器是 single-threaded。跨线程共享必须由调用方在容器 API 外同步；`const` 查询与 iterator 不隐含线程安全。初始化记录 `max_elements` 硬上限，所有节点大小、payload offset、alignment、capacity 和总字节计算都使用 checked arithmetic。到达上限返回 `SALTS_STL_CAPACITY_EXCEEDED`，OOM 返回 `SALTS_STL_OUT_OF_MEMORY`，二者不得混淆。
...
STLCFlow 以 Adapter + Strategy 实现这些接口：sequence state 使用 `salts_vec_t`，distinct state 使用 `salts_hash_set_t`，typed terminal 使用 CMeta collector。

不使用全局 registry、服务定位器或隐式弱 fallback。backend 由 `<cstl/cflow.h>` 在构造 stream 时显式注入。CFlow graph 只保存语义参数，不保存 Container 对象：
...
Container 提供稳定排序和 hash set 实现；CFlow 与 STLCFlow 不另写动态数组、hash table 或排序基础设施。以上复杂度是算法契约，不代表吞吐或缓存收益；性能结论必须来自实现后的 benchmark/profile。
...
```text
SALTS_STL_OK
SALTS_STL_INVALID_ARGUMENT
SALTS_STL_OUT_OF_MEMORY
SALTS_STL_CAPACITY_EXCEEDED
SALTS_STL_EMPTY
SALTS_STL_NOT_FOUND
SALTS_STL_TYPE_MISMATCH
SALTS_STL_TRAIT_MISSING
```
...
fluent intermediate method 继续返回 self，失败后 stream 进入 sticky failed 状态。terminal 返回 `cflow_status`；详细信息从 stream/run error accessor 读取。STLCFlow 在唯一适配边界把 `salts_stl_status` 转换为 `cflow_status`。中间层不重复记录日志，应用在消费或转换错误的边界决定是否记录。
...
- 安装树只包含新 headers/targets，新 consumer 通过 `find_package(Salts)` 构建。
...
2. 先构建并运行 CMeta、Container、CFlow、STLCFlow 最小相关 targets；
...
事实：旧 include、typed 宏和 `Salts::Stream` 被删除；List 不再提供 deque 容量/随机访问接口，Map 不再接受 hash/equal 或暴露 slot/capacity。影响所有直接消费者。验证安装 manifest、CMake export、compile-negative fixtures 和独立 consumer；在 release notes 给出一对一迁移表。本设计明确不提供兼容层。
...
- 标准容器文件只存在于 `cstl/`，Core 和旧 `salts/` 无重复实现；
- CFlow target 不依赖 Container，STLCFlow 是唯一适配实现；
