...
`tinytest/include/tinymeta/traits.h` 是 TinyTest 自己的编译期事实源。`TTEST_EQ`、`TTEST_NE`、
...
`tinytest_cmeta.h` 由 `tinytest.h` 在 `tinymeta/traits.h` 之后单向包含；它不是独立入口，且不包含或链接 CMeta。
