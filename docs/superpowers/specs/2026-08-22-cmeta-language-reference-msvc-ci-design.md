...
Further formal work is optional and must not block normal CMeta/CFlow product development unless a future feature depends on a property that is not already covered. In particular, the following remain future work:

- M8 root-module isolation and `CMeta.InternalChecks` packaging;
- MSVC nested-replay backend certification;
...
MSVC support is a portability/conformance lane, not a third formally certified preprocessor backend.
...
3. build portable CMeta/CFlow formal witness targets that do not encode GCC/Clang nested-replay certification;
4. run portable applicability/conformance executables;
5. install the pinned Lean toolchain and run `lake build --wfail` so the Lean model itself remains cross-platform buildable.

The MSVC lane must not generate or compare `NestedReplay*GeneratedC.lean` backend certificates. The existing GCC and Clang jobs remain the backend-certified lanes.
...
- making MSVC CI depend on MSVC-specific formal backend semantics before there is a concrete need.
