...
corpus, and its package-consumer test. TurboUtils still contains and exports
the older `cflow-scxml` copy as the optional `TurboUtils::CFlowScxml` target.
Publishing both copies creates two writable facts for one interpreter.

TurboUtils remains the owner of CFlow StateChart construction and execution,
...
consumer switch that publishes it from TurboUtils. TurboSCXML becomes the sole
SCXML implementation and package. TurboUtils documentation retains a short
ownership pointer but no duplicate SCXML API or behavior documentation.

The following TurboUtils interfaces are removed together:

- the `CFLOW_ENABLE_SCXML` CMake option;
- the `cflow-scxml/` source, public header, tests, fixtures, and corpus;
- the build-tree and installed `TurboUtils::CFlowScxml` target;
- `EXPECT_CFLOW_SCXML`, `TURBOUTILS_EXPECT_CFLOW_SCXML`, and the installed
...
### Keep a forwarding `TurboUtils::CFlowScxml` target

Rejected. TurboUtils cannot export a reliable alias to a package that is not a
TurboUtils dependency, and making TurboSCXML a TurboUtils dependency would
...
- Dependency direction is `TurboSCXML -> TurboUtils::CFlow`; CFlow never
...
- TurboUtils package size and its optional configuration matrix shrink; SCXML
  validation moves to the standalone repository.

## Compatibility and Migration

This is an intentional source and CMake package break for consumers of
`CFLOW_ENABLE_SCXML`, `TurboUtils::CFlowScxml`, or `<cflow/scxml.h>`.
...
1. A clean TurboUtils configure must not accept or export the removed target.
2. TurboUtils build, all `cflow_*` tests, and installed-package consumers must
...
4. TurboSCXML must configure against the newly installed TurboUtils package,
...
TurboSCXML sources back into TurboUtils is not an acceptable rollback because
