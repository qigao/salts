...
CMeta 计划作为整个应用的编译期元数据基础库。当前 `Salts::Core` 先于
`Salts::CMeta` 构建，`fmt.h` 与 `tlog.h` 又各自维护类型清单或枚举清单，因而
CMeta 还不能作为这些公共头文件的统一事实源。

本设计只处理第一个可验证切片：将现有的单一 CMeta 库提升为 Salts 的基础依赖，
并将 `salts_log_level_t` 迁移到 CMeta 的 `Enum` 描述。`fmt` 参数模型、字符串错误语义、
日志队列背压和异步内存布局留给后续独立切片。

## 目标

- 保持单一 `Salts::CMeta` 目标，不拆分头文件与运行时目标。
- 让 `Salts::Core` 通过公开依赖复用 CMeta 的头文件和实现。
...
- 不改变 `salts_log_level_from_name()` 对未知字符串的 INFO 回退语义。
- 不改变日志队列的阻塞、丢弃或关闭协议。
...
```text
Salts::Core ------> Salts::CMeta
```

根构建先加入 `cmeta/`，再加入 `utils/`。当前 `cmeta/src/cmeta.c` 只依赖 CMeta 自身
头文件和 C 标准库，不链接 `Salts::Core`，因此不会形成循环依赖。

现有 `salts_cmeta` 已随 `SaltsTargets` 安装和导出；`Salts::Core` 的 PUBLIC
链接关系会把唯一的 `Salts::CMeta` 目标传递给安装包使用者，不增加新导出目标。
...
生成的 `salts_log_level_t_meta()` 与 `salts_log_level_t_to_string()` 成为可用元数据接口。
现有 `salts_log_level_name()` 继续保留，内部委托给生成接口，并对非法值返回
`"UNKNOWN"`。

`salts_log_level_from_name()` 必须只匹配显示文本。虽然 CMeta 生成的
...
- `salts_log_level_t` 类型名和旧函数签名保持不变。
- CMeta 枚举描述符是头文件内静态对象；调用方只能依赖语义内容，不得把描述符地址
  当作跨翻译单元或插件边界的稳定 ID。
- `Salts::Core` 新增对 `Salts::CMeta` 的公开依赖；安装包使用者链接 Core 时会
  获得该传递依赖，无需手工重复声明。
