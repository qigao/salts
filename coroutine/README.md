...
- `try_submit*` 在 shard queue 满时返回 `TURBO_ENOBUFS`，阻塞提交只等待 queue admission；
- 成功 admission 恰好执行 `run` 或 `cancel`，随后执行可选 `finalize`；拒绝不调用 callback；
- `shutdown` 关闭 admission 并 drain 已接收的有限 cooperative coroutine；
- `turbo_coro_executor_yield()` 主动让出当前 shard；`coro_yield()` 仍可作为低层等价入口；
...
- completion 可以先于 `await` 到达，也可以在 `shutdown` 关闭 task admission 后到达。重复完成返回 `TURBO_EALREADY`，已消费或已 abort 的 handle 返回 `TURBO_ENOENT`。
