...
typedef struct {
  turbo_coro_executor_t *executor;
  turbo_coro_executor_task_t nested_task;
  atomic_int entered;
  atomic_int proceed;
  atomic_int returned;
  atomic_int status;
...
static void affinity_task(coro_t *coroutine, void *arg) {
  ...
  const size_t before = turbo_coro_executor_current_shard(state->executor);
  (void)coroutine;

  if (before != state->expected_shard)
    atomic_fetch_add_explicit(state->migrations, 1, memory_order_relaxed);
  coro_yield();
  if (turbo_coro_executor_current_shard(state->executor) != before)
    atomic_fetch_add_explicit(state->migrations, 1, memory_order_relaxed);
...
static void round_robin_task(coro_t *coroutine, void *arg) {
  ...
  const size_t shard = turbo_coro_executor_current_shard(state->executor);
  (void)coroutine;
...
static void blocking_submit_thread(void *arg) {
  ...
  state->status = turbo_coro_executor_submit_to(state->executor, 0u, &state->task);
...
static void reentrant_submit_task(coro_t *coroutine, void *arg) {
  ...
  atomic_store_explicit(&state->status,
                        turbo_coro_executor_submit_to(state->executor, 0u, &state->nested_task),
...
static void producer_thread(void *arg) {
  ...
  for (int index = 0; index < EXECUTOR_TEST_TASKS_PER_PRODUCER; ++index) {
    state->status = turbo_coro_executor_submit_to(state->executor, state->shard, &state->task);
...
static void await_task(coro_t *coroutine, void *arg) {
  ...
  state->before_shard = turbo_coro_executor_current_shard(state->executor);
  state->begin_status = turbo_coro_executor_await_begin(&state->await_handle);
  atomic_store_explicit(&state->handle_ready, 1, memory_order_release);
  if (state->begin_status != TURBO_OK) return;
  while (atomic_load_explicit(&state->allow_await, memory_order_acquire) == 0)
    turbo_coro_executor_yield();
  state->await_status = turbo_coro_executor_await(state->await_handle, &state->completion_status);
  state->after_shard = turbo_coro_executor_current_shard(state->executor);
...
static void await_abort_task(coro_t *coroutine, void *arg) {
  ...
  state->begin_status = turbo_coro_executor_await_begin(&state->await_handle);
  if (state->begin_status == TURBO_OK) {
    turbo_coro_executor_await_t rejected = {0};
    state->second_begin_status = turbo_coro_executor_await_begin(&rejected);
    state->abort_status = turbo_coro_executor_await_abort(state->await_handle);
  }
...
static void await_completion_producer(void *arg) {
  ...
  for (size_t index = state->first; index < EXECUTOR_TEST_AWAIT_COUNT; index += state->stride) {
    state->status = turbo_coro_executor_await_complete(
        state->executor, state->tasks[index].await_handle, (int)(100u + index));
    if (state->status != TURBO_OK) return;
  }
}

static int wait_for_waiting_awaits(turbo_coro_executor_t *executor, uint64_t expected) {
  for (int round = 0; round < EXECUTOR_TEST_WAIT_ROUNDS; ++round) {
    turbo_coro_executor_stats_t stats = {0};
    turbo_coro_executor_get_stats(executor, &stats);
...
static turbo_coro_executor_t *create_single_slot_executor(void) {
...
spec("Turbo coroutine executor") {
  it("rejects yield and await reservation outside an executor coroutine") {
    turbo_coro_executor_await_t await_handle = {(uintptr_t)9u, 9u, 9u, 9u, 9u};

    check_equal(turbo_coro_executor_yield(), TURBO_EINVAL);
    check_equal(turbo_coro_executor_await_begin(&await_handle), TURBO_EINVAL);
  ...
  it("routes an external completion to the same shard after shutdown") {
    turbo_coro_executor_config_t config = TURBO_CORO_EXECUTOR_CONFIG_DEFAULT;
    turbo_coro_executor_t *executor;
    turbo_coro_executor_t *other_executor;
    await_task_state state = {0};
    turbo_coro_executor_task_t task = {await_task, NULL, NULL, &state};
    turbo_coro_executor_stats_t stats = {0};
    ...
    check_equal(turbo_coro_executor_submit_to(executor, 1u, &task), TURBO_OK);
    check_true(wait_atomic_at_least(&state.handle_ready, 1));
    check_equal(turbo_coro_executor_await_complete(other_executor, state.await_handle, TURBO_OK),
                TURBO_EINVAL);
    check_equal(turbo_coro_executor_destroy(other_executor), TURBO_OK);
    atomic_store_explicit(&state.allow_await, 1, memory_order_release);
    check_true(wait_for_waiting_awaits(executor, 1u));
    check_equal(turbo_coro_executor_shutdown(executor), TURBO_OK);
    check_equal(turbo_coro_executor_await_complete(executor, state.await_handle, TURBO_ETIMEDOUT),
                TURBO_OK);
    check_equal(turbo_coro_executor_wait(executor), TURBO_OK);
    turbo_coro_executor_get_stats(executor, &stats);

    check_equal(state.begin_status, TURBO_OK);
    check_equal(state.await_status, TURBO_OK);
    check_equal(state.completion_status, TURBO_ETIMEDOUT);
    ...
    check_equal(turbo_coro_executor_destroy(executor), TURBO_OK);
  }

  it("consumes completion published before await without suspending") {
    turbo_coro_executor_t *executor = create_single_slot_executor();
    await_task_state state = {0};
    turbo_coro_executor_task_t task = {await_task, NULL, NULL, &state};

    check_not_null(executor);
    state.executor = executor;
    check_equal(turbo_coro_executor_submit(executor, &task), TURBO_OK);
    check_true(wait_atomic_at_least(&state.handle_ready, 1));
    check_equal(turbo_coro_executor_await_complete(executor, state.await_handle, TURBO_ECANCELED),
                TURBO_OK);
    check_equal(turbo_coro_executor_await_complete(executor, state.await_handle, TURBO_OK),
                TURBO_EALREADY);
    atomic_store_explicit(&state.allow_await, 1, memory_order_release);
    check_equal(turbo_coro_executor_wait(executor), TURBO_OK);
    ...
    check_equal(state.begin_status, TURBO_OK);
    check_equal(state.await_status, TURBO_OK);
    check_equal(state.completion_status, TURBO_ECANCELED);
    check_equal(atomic_load_explicit(&state.resumed, memory_order_acquire), 1);
    check_equal(turbo_coro_executor_await_complete(executor, state.await_handle, TURBO_OK),
                TURBO_ENOENT);
    check_equal(turbo_coro_executor_destroy(executor), TURBO_OK);
  ...
  it("accepts concurrent completion producers on one bounded wake queue") {
    ...
    turbo_coro_executor_task_t tasks[EXECUTOR_TEST_AWAIT_COUNT];
    await_completion_producer_state producers[EXECUTOR_TEST_PRODUCER_COUNT] = {0};
    turbo_thread_t threads[EXECUTOR_TEST_PRODUCER_COUNT] = {0};
    ...
    for (size_t index = 0u; index < EXECUTOR_TEST_AWAIT_COUNT; ++index) {
      ...
      tasks[index] = (turbo_coro_executor_task_t){await_task, NULL, NULL, &states[index]};
      check_equal(turbo_coro_executor_submit(executor, &tasks[index]), TURBO_OK);
    }
    for (size_t index = 0u; index < EXECUTOR_TEST_AWAIT_COUNT; ++index) {
    ...
    for (size_t index = 0u; index < EXECUTOR_TEST_PRODUCER_COUNT; ++index) {
      producers[index] = (await_completion_producer_state){executor, states, index,
                                                           EXECUTOR_TEST_PRODUCER_COUNT, TURBO_OK};
      check_equal(
          turbo_thread_create(&threads[index], await_completion_producer, &producers[index]),
          TURBO_OK);
    }
    for (size_t index = 0u; index < EXECUTOR_TEST_PRODUCER_COUNT; ++index) {
      check_equal(turbo_thread_join(&threads[index]), TURBO_OK);
      check_equal(producers[index].status, TURBO_OK);
    }
    check_equal(turbo_coro_executor_wait(executor), TURBO_OK);
    for (size_t index = 0u; index < EXECUTOR_TEST_AWAIT_COUNT; ++index) {
      check_equal(states[index].await_status, TURBO_OK);
  ...
  it("aborts an await when external operation submission fails") {
    ...
    turbo_coro_executor_task_t task = {await_abort_task, NULL, NULL, &state};

    check_not_null(executor);
    check_equal(turbo_coro_executor_submit(executor, &task), TURBO_OK);
    check_equal(turbo_coro_executor_wait(executor), TURBO_OK);
    check_equal(state.begin_status, TURBO_OK);
    check_equal(state.second_begin_status, TURBO_EBUSY);
    check_equal(state.abort_status, TURBO_OK);
  ...
  it("invalidates an unconsumed await when its task returns") {
    turbo_coro_executor_t *executor = create_single_slot_executor();
    await_abort_state state = {0};
    turbo_coro_executor_task_t task = {await_unconsumed_task, NULL, NULL, &state};
    turbo_coro_executor_stats_t stats = {0};
  ...
  it("keeps an explicitly assigned coroutine on one worker across yield") {
    ...
    turbo_coro_executor_task_t tasks[2];
    atomic_int runs = 0;
    ...
    executor = turbo_coro_executor_create(&config);
    check_not_null(executor);
    check_equal(turbo_coro_executor_current_shard(executor), SIZE_MAX);

    for (size_t shard = 0u; shard < 2u; ++shard) {
      states[shard] = (affinity_task_state){executor, shard, &runs, &migrations};
      tasks[shard] = (turbo_coro_executor_task_t){affinity_task, NULL, NULL, &states[shard]};
      check_equal(turbo_coro_executor_submit_to(executor, shard, &tasks[shard]), TURBO_OK);
    }
  ...
  it("round robins unpinned coroutines across workers") {
    ...
    turbo_coro_executor_task_t tasks[4];
    ...
    for (size_t index = 0u; index < 4u; ++index) {
      ...
      tasks[index] = (turbo_coro_executor_task_t){round_robin_task, NULL, NULL, &states[index]};
      check_equal(turbo_coro_executor_submit(executor, &tasks[index]), TURBO_OK);
    }
  ...
  it("accepts concurrent MPSC producers on bounded shard queues") {
    ...
    for (int index = 0; index < EXECUTOR_TEST_PRODUCER_COUNT; ++index) {
      ...
      producers[index].task = (turbo_coro_executor_task_t){lifecycle_task, NULL, NULL, &task_state};
      producers[index].shard = (size_t)index % 2u;
      check_equal(turbo_thread_create(&threads[index], producer_thread, &producers[index]),
                  TURBO_OK);
    }
    for (int index = 0; index < EXECUTOR_TEST_PRODUCER_COUNT; ++index) {
      check_equal(turbo_thread_join(&threads[index]), TURBO_OK);
  ...
  it("rejects a try submission when one shard queue is full") {
    ...
    turbo_coro_executor_task_t task = {gated_task, NULL, NULL, &state};

    check_not_null(executor);
    check_equal(turbo_coro_executor_submit_to(executor, 0u, &task), TURBO_OK);
    check_true(wait_atomic_at_least(&state.started, 1));
    check_equal(turbo_coro_executor_try_submit_to(executor, 0u, &task), TURBO_OK);
    check_equal(turbo_coro_executor_try_submit_to(executor, 0u, &task), TURBO_ENOBUFS);
  ...
  it("wakes a blocked submitter after queue capacity is released") {
    ...
    submitter.task = task;
    check_equal(turbo_thread_create(&thread, blocking_submit_thread, &submitter), TURBO_OK);
    ...
    check_equal(submitter.status, TURBO_OK);
    check_equal(turbo_coro_executor_wait(executor), TURBO_OK);
  ...
  it("wakes a blocked submitter with shutdown status when admission closes") {
    ...
    check_equal(turbo_coro_executor_shutdown(executor), TURBO_OK);
    check_equal(turbo_thread_join(&thread), TURBO_OK);
    check_equal(submitter.status, TURBO_ESHUTDOWN);
    atomic_store_explicit(&state.gate, 1, memory_order_release);
    check_equal(turbo_coro_executor_wait(executor), TURBO_OK);
    check_equal(atomic_load_explicit(&state.completed, memory_order_acquire), 2);
    check_equal(turbo_coro_executor_destroy(executor), TURBO_OK);
  ...
  it("rejects a blocking reentrant submission instead of deadlocking") {
    ...
    turbo_coro_executor_task_t outer_task = {reentrant_submit_task, NULL, NULL, &state};

    check_not_null(executor);
    state.executor = executor;
    state.nested_task = (turbo_coro_executor_task_t){lifecycle_task, NULL, NULL, &nested_state};
    check_equal(turbo_coro_executor_submit_to(executor, 0u, &outer_task), TURBO_OK);
    check_true(wait_atomic_at_least(&state.entered, 1));
    check_equal(turbo_coro_executor_try_submit_to(executor, 0u, &state.nested_task), TURBO_OK);
    ...
    check_equal(atomic_load_explicit(&state.status, memory_order_acquire), TURBO_EBUSY);
  ...
  it("drains accepted work and rejects submissions after shutdown") {
    ...
    check_equal(turbo_coro_executor_submit(executor, &task), TURBO_OK);
    check_equal(turbo_coro_executor_shutdown(executor), TURBO_OK);
    check_equal(turbo_coro_executor_try_submit(executor, &task), TURBO_ESHUTDOWN);
    check_equal(turbo_coro_executor_wait(executor), TURBO_OK);
  ...
  it("finalizes an accepted task exactly once after it runs") {
    ...
    turbo_coro_executor_task_t task = {lifecycle_task, lifecycle_cancel, lifecycle_finalize,
                                       &state};
    turbo_coro_executor_stats_t stats = {0};
  ...
  it("does not expose user data retained by a previously pooled task") {
    ...
    turbo_coro_executor_task_t task = {frame_isolation_task, NULL, NULL, &state};
  ...
  it("cancels and finalizes exactly once when frame allocation fails") {
    ...
    check_equal(atomic_load_explicit(&state.finalizes, memory_order_acquire), 1);
    check_equal(atomic_load_explicit(&state.cancel_status, memory_order_acquire), TURBO_ENOMEM);
    check_equal(stats.submitted_tasks, (uint64_t)1);
  ...
  it("rejects malformed bounded configurations") {
    ...
    check_null(turbo_coro_executor_create(&config));

    config = (turbo_coro_executor_config_t)TURBO_CORO_EXECUTOR_CONFIG_DEFAULT;
