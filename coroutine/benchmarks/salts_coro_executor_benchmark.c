...
static salts_coro_executor_t *create_bench_executor(size_t workers) {
...
static int run_bench_batch(salts_coro_executor_t *executor,
                           const salts_coro_executor_task_t *task) {
  for (int index = 0; index < CORO_EXECUTOR_TASKS_PER_SAMPLE; ++index) {
    if (salts_coro_executor_submit(executor, task) != SALTS_OK) return 0;
  }
  return salts_coro_executor_wait(executor) == SALTS_OK;
}

suite("Coroutine Executor Release benchmarks") {
  bench("compares one shard with bounded multi-shard execution") {
    salts_coro_executor_t *single = create_bench_executor(1u);
    salts_coro_executor_t *sharded;
    salts_coro_executor_task_t task = {cpu_yield_task, NULL, NULL, NULL};
    salts_coro_executor_stats_t single_stats = {0};
    salts_coro_executor_stats_t sharded_stats = {0};
    int cpu_count = salts_cpu_count();
    ...
    salts_coro_executor_get_stats(single, &single_stats);
    salts_coro_executor_get_stats(sharded, &sharded_stats);
    ...
    check_equal(salts_coro_executor_destroy(single), SALTS_OK);
    check_equal(salts_coro_executor_destroy(sharded), SALTS_OK);
  }
}
