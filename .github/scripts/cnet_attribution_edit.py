...
# The callback comparison is benchmark harness work, not CNet-internal validation.
text = text.replace("cnet_payload_validation_ns", "cnet_benchmark_payload_check_ns")
text = text.replace("payload_validation_ns", "benchmark_payload_check_ns")
text = text.replace("validation_started", "payload_check_started")
...
old = """  for (size_t repeat = 0u; repeat < IO_BENCH_REPLICATES; ++repeat) {
...
text = text.replace("const double validation_ns =", "const double payload_check_ns =")
text = text.replace(
    "callback_ns > validation_ns ? callback_ns - validation_ns : 0.0;",
    "callback_ns > payload_check_ns ? callback_ns - payload_check_ns : 0.0;",
)

marker = """static void io_bench_print_cnet_stages(const char *protocol, const io_bench_series *cnet,
                                       size_t count) {
"""
inserted = marker + """  printf("\\n%s CNet producer-side send attribution per call\\n", protocol);
...
"""
replace_once(marker, inserted)
text = text.replace("payload validation median ns", "benchmark payload check median ns")
text = text.replace("validation_ns", "payload_check_ns")
