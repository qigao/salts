...
$binaryRoundingBoundary = [pscustomobject]@{
  stage_timing=$true; stage_timing_version=3; io_operations=12800
  ...
  dispatch_ns=0; executor_ns=0; drive_residual_ns=96744140
  completion_ready_ns=0; wake_resume_ns=0; wait_residual_ns=189813324
  dispatch_mean_ns=0.0; executor_mean_ns=0.0
  drive_residual_mean_ns=7558.136; completion_ready_mean_ns=0.0
  wake_resume_mean_ns=0.0; wait_residual_mean_ns=14829.166
}
...
$incompleteDisabledStageTiming = [pscustomobject]@{
  stage_timing=$false; stage_timing_version=0; io_operations=0
  admission_ns=0; completion_drive_ns=0; drive_ns=0; wait_ns=0
  completion_process_ns=0; completion_residual_ns=0
  admission_mean_ns=0.0; completion_drive_mean_ns=0.0; drive_mean_ns=0.0
...
Assert-ThrowsLike {
  Assert-CflowStageTimingReport -Report $incompleteDisabledStageTiming
} "*missing dispatch_ns*" "disabled stage report requires current fields"
$unversionedStageTiming = [pscustomobject]@{
...
Assert-ThrowsLike {
  Assert-CflowStageTimingReport -Report $unversionedStageTiming
} "*missing stage_timing_version*" "stage report requires an explicit version"
...
$summary = Get-CflowStageTimingSummary -Reports $reports `
...
$handoffTable = Format-CflowHandoffReport `
  -Summaries @($summary) -Backends @("epoll") -Protocols @("tcp")
...
$zeroHandoffTable = Format-CflowHandoffReport `
  -Summaries @($zeroHandoffSummary) -Backends @("epoll") -Protocols @("tcp")
...
$splitHandoffTable = Format-CflowHandoffReport `
  -Summaries @($summary, $udpHandoffSummary) -Backends @("epoll")
...
$mixedProtocolSummary = Get-CflowStageTimingSummary `
...
$windowFourSummary = Get-CflowStageTimingSummary `
...
Assert-Equal $pipelineLayerSummary.paired_actor_direct_p50_ratio 1.15 `
  "pipeline Actor/direct P50 ratio"
Assert-Equal $pipelineLayerSummary.paired_source_direct_p50_ratio 1.5 `
  "pipeline Source/direct P50 ratio"
Assert-Equal $pipelineLayerSummary.paired_actor_direct_p95_ratio 1.0625 `
  "pipeline Actor/direct P95 ratio"
Assert-Equal $pipelineLayerSummary.paired_source_direct_p95_ratio 1.4375 `
  "pipeline Source/direct P95 ratio"
...
$largeSummary = Get-CflowStageTimingSummary -Reports $reports `
...
Assert-Throws {
  Get-CflowStageTimingSummary -Reports $missingPair `
    -Backend epoll -Protocol tcp -WaitMode blocking -PayloadBytes 64 `
    -ExpectedRuns 3
} "missing Source pair"
...
Assert-Throws {
  Get-CflowStageTimingSummary -Reports $duplicatePair `
    -Backend epoll -Protocol tcp -WaitMode blocking -PayloadBytes 64 `
    -ExpectedRuns 3
...
Assert-Throws {
  Get-CflowStageTimingSummary -Reports $mismatchedAttempts `
    -Backend epoll -Protocol tcp -WaitMode blocking -PayloadBytes 64 `
    -ExpectedRuns 3
} "mismatched stage timing attempt counts"
...
Assert-Equal $ioModelSummary.paired_adapter_direct_cost_ratio 6.5 `
  "mock IO Source adapter/direct cost ratio"
Assert-Equal $ioModelSummary.paired_runtime_direct_cost_ratio 8.0 `
  "mock Source runtime/direct cost ratio"
Assert-Equal $ioModelSummary.paired_coroutine_direct_cost_ratio 9.0 `
  "mock coroutine Source adapter/direct cost ratio"
Assert-Equal $ioModelSummary.paired_actor_direct_delta_ns 50.0 `
  "mock Actor/direct absolute cost"
Assert-Equal $ioModelSummary.paired_adapter_direct_delta_ns 75.0 `
  "mock IO Source adapter/direct absolute cost"
Assert-Equal $ioModelSummary.paired_runtime_direct_delta_ns 95.0 `
  "mock Source runtime/direct absolute cost"
Assert-Equal $ioModelSummary.paired_coroutine_direct_delta_ns 110.0 `
  "mock coroutine Source adapter/direct absolute cost"
Assert-Throws {
...
$missingCreditIoSourceOutput = @($ioSourceOutput | ForEach-Object {
...
Assert-ThrowsLike {
  ConvertFrom-CflowIoSourceBenchmarkOutput `
    -Lines $missingCreditIoSourceOutput -ExpectedCapacity 1 -ExpectedSamples 2 `
    -ExpectedValuesPerSample 4 -BenchmarkRun 1
} "*pending_drive_credit*" "IO Source report requires drive credit"
...
$transportComparisons = @($transportCases | ForEach-Object {
    [pscustomobject]@{
      ...
      paired_actor_direct_p50_ratio = 3.0
      paired_source_direct_p50_ratio = 5.0
      paired_actor_direct_p99_ratio = 2.0
      paired_source_direct_p99_ratio = 3.0
      paired_actor_direct_echo_ratio = 0.8
      paired_source_direct_echo_ratio = 0.7
      paired_actor_direct_application_mib_ratio = 0.8
      paired_source_direct_application_mib_ratio = 0.7
    }
  })
...
Assert-Equal ($transportReports.latency -match
    '\| 32 KiB \| Direct \| 10\.000 \| 1\.000 \| 20\.000 \| 1\.000 \|') $true `
  "UDP 32 KiB latency row"
Assert-Equal ($transportReports.latency -match
    '\| 65,507 B \(max datagram\) \| Direct \| 10\.000 \| 1\.000 \| 20\.000 \| 1\.000 \|') `
  $true "UDP maximum datagram latency row"
Assert-Equal ($transportReports.throughput -match
    '\| 32 KiB \| Direct \| 2 \| 1,000\.000 \| 1\.000 \| 10\.000 \| 1\.000 \|') $true `
  "UDP 32 KiB throughput row"
Assert-Equal ($transportReports.throughput -match
    '\| 65,507 B \(max datagram\) \| Direct \| 1 \| 1,000\.000 \| 1\.000 \| 10\.000 \| 1\.000 \|') `
...
Assert-Equal ($transportReports.latency -match 'P50/Direct') $true `
  "transport latency exposes Direct-relative P50"
Assert-Equal ($transportReports.throughput -match 'MiB/s/Direct') $true `
  "transport throughput exposes Direct-relative throughput"
