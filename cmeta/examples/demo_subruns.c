...
int main(void) {
    ...
    if (!cflow_source_from_array(&src, &cmeta_type_int, &one, 1)) return 3;
    if (!cflow_run_open(&run, &exec, &src, &loop, &obs)) return 4;
    if (!cflow_run_request(&run, 1)) return 5;
    ...
    printf("subrun ZIP: parent -> child -> parent scheduler handoff, value=%.2f\n", st.values[0]);
    cflow_run_close(&run);
    ...
    if (!cflow_source_from_array(&src, &cmeta_type_int, &one, 1)) return 20;
    if (!cflow_run_open(&run, &exec, &src, &loop, &obs)) return 21;
    if (!cflow_run_request(&run, 1)) return 22;
    if (!cflow_scheduler_run_one(&loop)) return 23; /* waiting on child */
    cflow_run_cancel(&run);
    (void)cflow_scheduler_run_until_idle(&loop, 0);
    if (!cflow_run_is_cancelled(&run) || atomic_load(&st.count) != 0 ||
        atomic_load(&st.done) || atomic_load(&st.errors)) return 24;
    printf("subrun cancellation: parent cancel propagates into child\n");
    cflow_run_close(&run);
    cflow_scheduler_destroy(&loop);
    cflow_graph_destroy(&exec); exec = (cflow_graph){0}; exec.root = CMETA_INVALID_ID;
    cflow_stream_destroy(&left); cflow_stream_destroy(&right);
    ...
    if (!cflow_source_from_array(&src, &cmeta_type_int, &one, 1)) return 26;
    if (!cflow_run_open(&run, &exec, &src, &loop, &obs)) return 27;
    if (!cflow_run_request(&run, 1)) return 28;
    (void)cflow_scheduler_run_until_idle(&loop, 0);
    if (cflow_run_error(&run) || atomic_load(&st.count) != 1 ||
    ...
    /* Same subrun composition on a concurrent scheduler. */
    build_zip(&left, &right);
    ...
    if (!cflow_source_from_array(&src, &cmeta_type_int, many, 3)) return 13;
    if (!cflow_run_open(&run, &exec, &src, &workers, &obs)) return 14;
    if (!cflow_run_request(&run, 3)) return 15;
    ...
    printf("subrun ZIP: concurrent scheduler, serialized outputs %.2f %.2f %.2f\n",
    ...
    if (!cflow_source_from_array(&src, &cmeta_type_int, nested_input, 3)) return 31;
    if (!cflow_run_open(&run, &exec, &src, &workers, &obs)) return 32;
    if (!cflow_run_request(&run, 3)) return 33;
    ...
    printf("nested subruns: parent -> subrun -> nested subrun composition %.2f %.2f %.2f\n",
           st.values[0], st.values[1], st.values[2]);
    cflow_run_close(&run);
    cflow_scheduler_destroy(&workers);
