#ifndef CFLOW_FS_WATCH_SOURCE_H
#define CFLOW_FS_WATCH_SOURCE_H
...
/* Move-only cleanup owner. Keep it alive until the Source is destroyed. */
typedef struct cflow_fs_watch_source_owner {
  void *impl;
} cflow_fs_watch_source_owner;
...
typedef struct cflow_fs_watch_source_config {
  bool recursive;
  size_t event_capacity;
  size_t watch_capacity;
  size_t path_capacity;
  size_t native_buffer_capacity;
  /* name, output_type, and encode_user remain borrowed through Source destruction. */
  const char *name;
  const cmeta_type_desc *output_type;
  cflow_fs_watch_encode_fn encode;
  void *encode_user;
} cflow_fs_watch_source_config;

/**
 * Open a native watcher and expose its events as a typed CFlow Source.
 *
 ...
 *
 * Success moves cleanup responsibility into Source plus owner. Destroy the
 * Source first, then retry owner_close while it returns TURBO_EBUSY. Failure
 * leaves both caller-owned outputs unchanged.
 *
 * @param out Zero-initialized Source destination, moved into Run on success.
...
/**
 * Drain and release native state after Source destruction.
 * Returns TURBO_EBUSY while Source is live or native shutdown is incomplete.
