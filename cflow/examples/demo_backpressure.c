...
int main(void) {
    ...
    int input = 7; cflow_source source; cflow_source_from_array(&source, &cmeta_type_int, &input, 1);
    cflow_scheduler loop; cflow_scheduler_test_init(&loop);
    state st = {0}; cflow_sink_callbacks obs_cb = { on_value, on_error, on_done, &st };
