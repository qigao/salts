...
typedef int (*file_open_fn)(cflow_io_file *, const char *,
                            const cflow_io_file_config *);
typedef bool (*file_supported_fn)(
    const cflow_io_file *, cflow_io_native_file_operation_kind);
typedef cflow_io_file_submit_result (*file_read_fn)(
    cflow_io_file *, cflow_io_lease_id, void *, size_t, uint64_t);
typedef cflow_io_file_submit_result (*file_write_fn)(
    cflow_io_file *, cflow_io_lease_id, const void *, size_t, uint64_t);
typedef cflow_io_file_submit_result (*file_flush_fn)(
    cflow_io_file *, cflow_io_lease_id);
typedef cflow_io_cancel_status (*file_cancel_fn)(
    cflow_io_file *, cflow_io_request_id);
...
static void file_completion(void *user,
    ...
    if (probe != NULL && completion != NULL &&
        probe->count < FILE_TEST_COMPLETION_CAPACITY) {
    ...
    if (probe != NULL && probe->reentrant_file != NULL) {
        ...
        probe->reentrant_status = cflow_io_file_run_ready(
            file, 1u, &probe->reentrant_progressed);
    }
}
...
static int file_test_wait(cflow_io_file *file,
                          file_completion_probe *probe,
    ...
    for (;;) {
        ...
        if (!cflow_io_file_get_stats(file, &stats))
            return TURBO_EINVAL;
        if (probe->count >= expected_count &&
            stats.operation_slots_in_use == 0u)
            return TURBO_OK;
        if (turbo_hrtime() - started >= FILE_TEST_TIMEOUT_NS)
...
spec("CFlow async file facade") {
    it("exposes the bounded public contract") {
        ...
        check_true(_Generic(&cflow_io_file_open,
                            file_open_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_operation_supported,
                            file_supported_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_try_read_at,
                            file_read_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_try_write_at,
                            file_write_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_try_flush,
                            file_flush_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_try_cancel,
                            file_cancel_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_run_ready,
                            file_run_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_close,
                            file_close_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_is_quiescent,
                            file_quiescent_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_get_stats,
                            file_stats_fn: true, default: false));
        check_true(_Generic(&cflow_io_file_destroy,
                            file_destroy_fn: true, default: false));
    }

    it("rejects malformed open configurations without publishing state") {
        char *path = tt_make_temp_file("cflow-io-file-config-", ".bin");
        cflow_io_file file = {0};
        cflow_io_file_config config = file_test_config(
    ...
    it("rejects unsupported file backends before creating the path") {
        ...
        cflow_io_file file = {0};
        cflow_io_file_config config = file_test_config(
            CFLOW_IO_FILE_WRITE | CFLOW_IO_FILE_CREATE |
    ...
    it("owns a compatible handle until close and quiescent destroy") {
        ...
        check_true(cflow_io_file_operation_supported(
            &file, CFLOW_IO_NATIVE_FILE_READ_AT));
        check_true(cflow_io_file_operation_supported(
            &file, CFLOW_IO_NATIVE_FILE_WRITE_AT));
    ...
    it("round-trips explicit-offset writes and reads") {
        ...
        submitted = cflow_io_file_try_write_at(
            &file, 11u, payload, sizeof(payload) - 1u, 7u);
        ...
        submitted = cflow_io_file_try_read_at(
            &file, 12u, received, sizeof(received), 7u);
        check_equal(submitted.status, CFLOW_IO_FILE_SUBMIT_ACCEPTED);
    ...
    it("flushes only when the selected native backend supports it") {
        ...
        if (cflow_io_file_operation_supported(
                &file, CFLOW_IO_NATIVE_FILE_FLUSH)) {
            check_equal(submitted.status, CFLOW_IO_FILE_SUBMIT_ACCEPTED);
            check_equal(file_test_wait(&file, &probe, 1u), TURBO_OK);
    ...
    it("rejects invalid shapes access mismatches and unsupported flush") {
        ...
        check_equal(submitted.status, CFLOW_IO_FILE_SUBMIT_INVALID_ARGUMENT);
        submitted = cflow_io_file_try_read_at(
            &file, 1u, &byte, 2u, (uint64_t)INT64_MAX);
        check_equal(submitted.status, CFLOW_IO_FILE_SUBMIT_INVALID_ARGUMENT);
    ...
    it("applies bounded backpressure and reuses acknowledged slots") {
        ...
        check_equal(status, TURBO_OK);
        submitted = cflow_io_file_try_write_at(
            &file, 21u, first, sizeof(first) - 1u, 0u);
        check_equal(submitted.status, CFLOW_IO_FILE_SUBMIT_ACCEPTED);
        submitted = cflow_io_file_try_write_at(
            &file, 22u, second, sizeof(second) - 1u, 16u);
    ...
    it("cancels queued work during close before destroying the handle") {
        ...
        check_equal(status, TURBO_OK);
        submitted = cflow_io_file_try_write_at(
            &file, 31u, payload, sizeof(payload) - 1u, 0u);
        check_equal(submitted.status, CFLOW_IO_FILE_SUBMIT_ACCEPTED);
        check_equal(cflow_io_file_close(&file), TURBO_OK);
        check_equal(cflow_io_file_destroy(&file), TURBO_EBUSY);
        check_equal(file_test_wait(&file, &probe, 1u), TURBO_OK);
    ...
    it("delivers one terminal completion for explicit cancellation") {
        ...
        check_equal(status, TURBO_OK);
        submitted = cflow_io_file_try_write_at(
            &file, 41u, payload, sizeof(payload) - 1u, 0u);
        check_equal(submitted.status, CFLOW_IO_FILE_SUBMIT_ACCEPTED);
        check_equal(cflow_io_file_try_cancel(&file, submitted.request_id),
