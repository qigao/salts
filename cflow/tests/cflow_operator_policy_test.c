...
suite("CFlow generated operator policy") {
    it("preserves built-in operator cardinalities") {
        check_equal((size_t)CFLOW_BUILTIN_FILTER_SIGNATURE_COUNT, (size_t)2u);
        check_equal((size_t)CFLOW_BUILTIN_MAP_SIGNATURE_COUNT, (size_t)8u);
        ...
        check_equal((size_t)CFLOW_BUILTIN_REDUCE_SIGNATURE_COUNT, (size_t)2u);
    ...
    it("admits built-ins only for their intended operators") {
        ...
        check_false(cflow_op_signature_allowed(CFLOW_OP_MAP,
                                               CMETA_SIG_U_I_B));
        check_false(cflow_op_signature_allowed(CFLOW_OP_FILTER,
                                               CMETA_SIG_U_D_D));
    }
}
