...
#define CFLOW_RANGE_BENCH_PAIR(label, sample_count, item_count)                    \
  do {                                                                            \
    ...
    check_true(cflow_range_bench_eval(&fixture.sized, (item_count)));               \
    benchmark_ops("Range result / unsized / " label, (sample_count),               \
                  (item_count)) {                                                  \
      unsized_ok = cflow_range_bench_eval(&fixture.unsized, (item_count));          \
    }                                                                              \
    check_true(unsized_ok);                                                        \
    benchmark_ops("Range result / SIZED flag / " label, (sample_count),            \
                  (item_count)) {                                                  \
...
suite("CFlow Range collection benchmarks") {
  bench("tracks unsized and SIZED Range collection baselines") {
