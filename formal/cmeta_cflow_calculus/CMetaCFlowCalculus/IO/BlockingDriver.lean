...
def project (state : State) : Communication.Contract :=
  { capacity := state.capacity
    ...
    events := state.events
    pending := state.pending.map Job.identity
...
def tryExecute (state : State) (identity : Communication.Identity)
    (job : Nat) : AdmissionResult :=
  if job = 0 then
    ...
    | .open =>
        if identity ∈ state.pending.map Job.identity then
...
def startJob : List Job → Communication.Identity → List Job
  | [], _ => []
  | current :: remaining, identity =>
      if current.identity = identity then
        { current with phase := .running } :: remaining
      else current :: startJob remaining identity

def start (state : State) (identity : Communication.Identity) : State :=
  { state with pending := startJob state.pending identity }

def eraseJob : List Job → Communication.Identity → List Job
  | [], _ => []
  | current :: remaining, identity =>
      if current.identity = identity then remaining
      else current :: eraseJob remaining identity
...
def tryComplete (state : State) (event : Communication.Event) :
  ...
  if ¬event.Valid then
    { status := .invalid, state := state }
  else if event.identity ∉ state.pending.map Job.identity then
    ...
    | .accepted =>
        { status := .accepted
          state :=
            { state with
              events := published.2
              pending := eraseJob state.pending event.identity } }
    | .full => { status := .full, state := state }
    | .closed => { status := .closed, state := state }
