...
int chttp_h2_frame_header_encode(uint8_t *output, size_t capacity, size_t *output_size,
  ...
  if (output == NULL || output_size == NULL) return TURBO_EINVAL;
  if (capacity < CHTTP_H2_FRAME_HEADER_SIZE) return TURBO_EMSGSIZE;
  if (length > 0x00ffffffu || (stream_id & 0x80000000u) != 0u) return TURBO_EINVAL;
...
int chttp_h2_frame_header_decode(const uint8_t *input, size_t input_size, size_t *consumed_size,
  ...
  uint32_t length;
  if (input == NULL || consumed_size == NULL || header == NULL) return TURBO_EINVAL;
  if (input_size < CHTTP_H2_FRAME_HEADER_SIZE) return TURBO_EMSGSIZE;

  length = chttp_h2_read_u24(input);
  if (length > max_frame_size) return TURBO_EMSGSIZE;
...
int chttp_h2_frame_data_payload(const uint8_t *payload, size_t payload_size, uint8_t flags,
  ...
  size_t padding_size;
  if (payload == NULL || data == NULL || data_size == NULL) return TURBO_EINVAL;
  if ((flags & CHTTP_H2_FLAG_PADDED) == 0u) {
    *data = payload;
    *data_size = payload_size;
    return TURBO_OK;
  }
  if (payload_size == 0u) return TURBO_EPROTO;
  padding_size = payload[0];
  if (padding_size > payload_size - 1u) return TURBO_EPROTO;
  *data = payload + 1u;
...
int chttp_h2_frame_headers_payload(const uint8_t *payload, size_t payload_size, uint8_t flags,
  ...
  if (payload == NULL || block == NULL || block_size == NULL) return TURBO_EINVAL;

  if ((flags & CHTTP_H2_FLAG_PADDED) != 0u) {
    if (payload_size == 0u) return TURBO_EPROTO;
    padding_size = payload[0];
    offset = 1u;
    if (padding_size > payload_size - offset) return TURBO_EPROTO;
  }

  if ((flags & CHTTP_H2_FLAG_PRIORITY) != 0u) {
    if (payload_size - offset - padding_size < 5u) return TURBO_EPROTO;
...
int chttp_h2_frame_settings_encode(uint8_t *output, size_t capacity, size_t *output_size,
  ...
  if (output == NULL || output_size == NULL ||
    ...
    return TURBO_EINVAL;
  if (count > SIZE_MAX / 6u) return TURBO_EMSGSIZE;
  required_size = count * 6u;
  if (capacity < required_size) return TURBO_EMSGSIZE;

  for (index = 0u; index < count; ++index) {
    if (identifiers[index] > UINT16_MAX) return TURBO_EINVAL;
...
int chttp_h2_frame_settings_parse(const uint8_t *payload, size_t payload_size,
  ...
  size_t index;
  size_t parsed_count;
  if (payload == NULL || count == NULL) return TURBO_EINVAL;
  if (payload_size % 6u != 0u) return TURBO_EPROTO;
  parsed_count = payload_size / 6u;
  if (parsed_count > max_count) return TURBO_EMSGSIZE;
...
int chttp_h2_frame_rst_encode(uint8_t *output, size_t capacity, size_t *output_size,
                              uint32_t error_code) {
  if (output == NULL || output_size == NULL) return TURBO_EINVAL;
  if (capacity < 4u) return TURBO_EMSGSIZE;
...
int chttp_h2_frame_goaway_encode(uint8_t *output, size_t capacity, size_t *output_size,
  ...
  if (output == NULL || output_size == NULL) return TURBO_EINVAL;
  if (capacity < 8u) return TURBO_EMSGSIZE;
  if ((last_stream_id & 0x80000000u) != 0u) return TURBO_EINVAL;
...
int chttp_h2_frame_goaway_parse(const uint8_t *payload, size_t payload_size,
                                uint32_t *last_stream_id, uint32_t *error_code) {
  if (payload == NULL || last_stream_id == NULL || error_code == NULL) return TURBO_EINVAL;
  if (payload_size < 8u) return TURBO_EMSGSIZE;
...
int chttp_h2_frame_ping_encode(uint8_t *output, size_t capacity, size_t *output_size,
  ...
  if (output == NULL || output_size == NULL || opaque_bytes == NULL) return TURBO_EINVAL;
  if (capacity < 8u) return TURBO_EMSGSIZE;
...
int chttp_h2_frame_priority_encode(uint8_t *output, size_t capacity, size_t *output_size,
  ...
  if (output == NULL || output_size == NULL || weight == 0u || weight > 256u ||
    ...
    return TURBO_EINVAL;
  if (capacity < 5u) return TURBO_EMSGSIZE;
